...
class trx_addons_demo_data_importer {
	...
	function prepare_vars() {
		...
		$this->options['debug'] = trx_addons_is_on(trx_addons_get_theme_option('debug_mode'));
	...
	function admin_scripts() {
		wp_enqueue_style(  'trx_addons-importer',  TRX_ADDONS_PLUGIN_URL . 'importer/importer.css', array(), null );
		wp_enqueue_script( 'trx_addons-importer', TRX_ADDONS_PLUGIN_URL . 'importer/importer.js', array('jquery'), null, true );
	}
		...
		?><div class="trx_importer"><?php
			...
			?><div class="trx_importer_section">
				...
				<form id="trx_importer_form">
					...
					$this->show_importer_params(array(
						'slug' => 'to',
						'title' => esc_html__('Import Theme Settings', 'trx_addons'),
						'part' => 1,
						'checked' => true
					));
					$this->show_importer_params(array(
				...
				<div id="trx_importer_progress" class="notice notice-info style_<?php echo esc_attr($this->options['demo_style']); ?>">
					...
					$fields = array(
						...
						'to'		=> esc_html__('Theme Settings', 'trx_addons'),
				...
				<form id="trx_exporter_form" action="#" method="post">
					...
					if ( isset($_POST['exporter_action']) ) { 
						...
						$fields = array(
							...
							'theme_options'	=> esc_html__('Theme Settings', 'trx_addons'),
							'widgets'		=> esc_html__('Widgets', 'trx_addons'),
						);
						foreach ($fields as $slug=>$title) {
	...
	function show_exporter_fields($args=array()) {
		...
		<tr>
			...
			<td><a download="<?php echo esc_attr(!empty($args['download']) ? $args['download'] : $args['slug'].'.txt'); ?>" href="<?php echo esc_url(TRX_ADDONS_PLUGIN_URL . 'importer/export/'.trim($args['slug']).'.txt'); ?>"><?php esc_html_e('Download', 'trx_addons'); ?></a></td>
		</tr>
		<?php
	}
	...
	function exporter() {
		...
		trx_addons_fpc(TRX_ADDONS_PLUGIN_DIR . 'importer/export/theme_mods.txt', serialize($this->prepare_data(apply_filters('trx_addons_filter_export_mods', get_theme_mods()))));
		...
		trx_addons_fpc(TRX_ADDONS_PLUGIN_DIR . 'importer/export/theme_options.txt', serialize($this->prepare_data(apply_filters('trx_addons_filter_export_options', $options))));
		...
		trx_addons_fpc(TRX_ADDONS_PLUGIN_DIR . 'importer/export/widgets.txt', serialize($this->prepare_data(apply_filters('trx_addons_filter_export_widgets', $options))));

		// Export posts
		trx_addons_fpc(TRX_ADDONS_PLUGIN_DIR . 'importer/export/posts.txt', serialize(array(
		...
		$users['usermeta'] = $rows;
		trx_addons_fpc(TRX_ADDONS_PLUGIN_DIR . 'importer/export/users.txt', serialize($users));
	...
	function export_dump($table) {
		...
		if ( count( $wpdb->get_results( $wpdb->prepare( "SHOW TABLES LIKE %s", $wpdb->prefix . trim($table) ), ARRAY_A ) ) == 1 ) {
			...
			if ($this->options['debug']) dfl(sprintf(__("Export %d rows from table '%s'. Used query: %s", 'trx_addons'), count($rows), $table, $query));
		}
		return $rows;
	}
	...
	function importer() {
		...
		if ( !isset($_POST['importer_action']) || !wp_verify_nonce( trx_addons_get_value_gp('ajax_nonce'), 'ajax_nonce' ) || !current_user_can('manage_options') )
			die();
		...
		foreach ($files as $file) {
			...
			if ($need_extract) {
				if (!empty($zip) && file_exists($zip)) {
					...
					if ( is_wp_error($rez) ) {
						$msg = sprintf(__('Error when unzip file "%s"', 'trx_addons'), $zip);
					...
					$msg = sprintf(__('File "%s" not found', 'trx_addons'), $zip);
					$this->response['error'] = $msg;
					if ($this->options['debug']) 
						dfl($msg);
				}
	...
	function import_theme_options() {
		if ($this->options['debug']) 
			dfl(__('Import Theme Settings', 'trx_addons'));
	...
	function import_dump($slug, $title) {
		...
		if (is_array($data) && count($data) > 0) {
			...
			if ($this->options['debug']) 
				dfl(sprintf(__('Error unserialize data from the file %s', 'trx_addons'), $this->options['files'][$this->options['demo_type']]['file_with_' . $slug]));
		}
		$this->response['result'] = $result;
	}
	...
	function get_list_pages_callback() {
		if ( !wp_verify_nonce( trx_addons_get_value_gp('ajax_nonce'), 'ajax_nonce' ) )
			die();
