/* global jQuery:false */
/* global TRX_ADDONS_GLOBALS:false */
...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_demo_type input[type="radio"]').on('change', function() {
		...
		var data = {
			ajax_nonce: TRX_ADDONS_GLOBALS['ajax_nonce'],
			action: 'trx_addons_importer_get_list_pages',
			demo_type: type
		};
		jQuery.post(TRX_ADDONS_GLOBALS['ajax_url'], data, function(response) {
			var rez = {};
			try {
				rez = JSON.parse(response);
			} catch (e) {
				rez = { error: TRX_ADDONS_GLOBALS['ajax_error']+':<br>'+response };
	...
	jQuery('.trx_importer_demo_set input[type="radio"]').on('change', function() {
        ...
        if (set == 'full' && !confirm(THEMEREX_GLOBALS['msg_importer_full_alert'])) {
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		TRX_ADDONS_GLOBALS['importer_error_messages'] = '';
		TRX_ADDONS_GLOBALS['importer_ignore_errors'] = true;
	...
	function trx_addons_importer_do_action(steps, idx) {
		...
		jQuery.post(TRX_ADDONS_GLOBALS['ajax_url'], data, function(response) {
			...
			if (rez.error === '' || TRX_ADDONS_GLOBALS['importer_ignore_errors']) {
				if (rez.error !== '') 
					TRX_ADDONS_GLOBALS['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
				...
				if (idx < steps.length) {
					...
					if (TRX_ADDONS_GLOBALS['importer_error_messages']) {
						jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + TRX_ADDONS_GLOBALS['msg_importer_error'] + '</h4>' + TRX_ADDONS_GLOBALS['importer_error_messages']);
					} else {
