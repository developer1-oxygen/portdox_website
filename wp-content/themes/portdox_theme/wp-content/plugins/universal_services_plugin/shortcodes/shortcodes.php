...
function themerex_sc_accordion($atts, $content=null){	
	...
	$THEMEREX_GLOBALS['sc_accordion_icon_closed'] = empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed) ? "icon-down-open" : $icon_closed;
	$THEMEREX_GLOBALS['sc_accordion_icon_opened'] = empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened) ? "icon-down-open" : $icon_opened;
...
function themerex_sc_accordion_item($atts, $content=null) {
	...
	if (empty($icon_closed) || themerex_sc_param_is_inherit($icon_closed)) $icon_closed = $THEMEREX_GLOBALS['sc_accordion_icon_closed'] ? $THEMEREX_GLOBALS['sc_accordion_icon_closed'] : "icon-down-open";
	if (empty($icon_opened) || themerex_sc_param_is_inherit($icon_opened)) $icon_opened = $THEMEREX_GLOBALS['sc_accordion_icon_opened'] ? $THEMEREX_GLOBALS['sc_accordion_icon_opened'] : "icon-down-open";
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
...
themerex_require_shortcode("trx_anchor", "themerex_sc_anchor");
...
/*
[trx_audio url="http://globallogistics.themerex.net/wp-content/uploads/2014/12/Dream-Music-Relax.mp3" image="http://globallogistics.themerex.dnw/wp-content/uploads/2014/10/post_audio.jpg" title="Insert Audio Title Here" author="Lily Hunter" controls="show" autoplay="off"]
*/
	...
	if (!themerex_sc_param_is_off($filters)) {
		...
		if ($filters == 'categories') {			// Use categories as filter items
			...
			if (count($portfolio_list) > 0) {
				$filters_list .= '<a href="#" data-filter="*" class="theme_button active">'.__('All', 'trx_addons').'</a>';
...
function themerex_sc_button($atts, $content=null){	
	...
	$output = '<a href="' . (empty($link) ? '#' : $link) . '"'
		...
		. ($bg_style == 'underline' ?
			...
			.($icon!='' && $icon!='inherit' ?
				'<span'
					. ($icon!='' && $icon!='inherit' ? ' class="sc_button_iconed '. esc_attr($icon) .'"'  : '')
					.'></span>'
				: '')
		 :
			'<span'
...
function themerex_sc_contact_form($atts, $content = null) {
	...
	if($style == 'topic' && themerex_sc_param_is_off($custom)){

		$output = '<div '
			. ' class="sc_contact_form sc_contact_form_topic'
			...
			. '<div class="sc_contact_info">'
				...
				. ($title ? '<h4 class="sc_contact_title">' . ($title) . '</h4>' : '')
			. '</div>'
			...
			. ($content != '' && themerex_sc_param_is_on($custom)
				? $content
				: '<div class="sc_contact_form_info_first">'

					. '<div class="columns_wrap sc_columns columns_nofluid sc_columns_count_2">'
						. '<div class="column-1_2 sc_column_item sc_column_item_1 odd first">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_username">' . __('First Name', 'trx_addons') . '</label><input id="sc_contact_form_username" type="text" name="username" placeholder="' . __('First Name', 'trx_addons') . '"></div>'
						. '</div>'
						. '<div class="column-1_2 sc_column_item sc_column_item_2 even">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_username2">' . __('Last Name', 'trx_addons') . '</label><input id="sc_contact_form_username2" type="text" name="username2" placeholder="' . __('Last Name', 'trx_addons') . '"></div>'
						. '</div>'
					. '</div>'
					...
					. '<div class="columns_wrap sc_columns columns_nofluid sc_columns_count_2">'
						...
						. '</div>'
						. '<div class="column-1_2 sc_column_item sc_column_item_2 even">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_company">' . __('Company', 'trx_addons') . '</label><input id="sc_contact_form_company" type="text" name="company" placeholder="' . __('Company', 'trx_addons') . '"></div>'
						. '</div>'
					. '</div>'

					. '<div class="columns_wrap sc_columns columns_nofluid sc_columns_count_2">'
						. '<div class="column-1_2 sc_column_item sc_column_item_1 odd first">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_address">' . __('Address', 'trx_addons') . '</label><input id="sc_contact_form_address" type="text" name="address" placeholder="' . __('Address', 'trx_addons') . '"></div>'
						. '</div>'
						. '<div class="column-1_2 sc_column_item sc_column_item_2 even">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_city">' . __('City', 'trx_addons') . '</label><input id="sc_contact_form_city" type="text" name="city" placeholder="' . __('City', 'trx_addons') . '"></div>'
						. '</div>'
					. '</div>'

					. '<div class="columns_wrap sc_columns columns_nofluid sc_columns_count_2">'
						. '<div class="column-1_2 sc_column_item sc_column_item_1 odd first">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_zip">' . __('Zip', 'trx_addons') . '</label><input id="sc_contact_form_zip" type="text" name="zip" placeholder="' . __('Zip', 'trx_addons') . '"></div>'
						. '</div>'
						. '<div class="column-1_2 sc_column_item sc_column_item_2 even">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_phone">' . __('Telephone (include country code)', 'trx_addons') . '</label><input id="sc_contact_form_phone" type="text" name="phone" placeholder="' . __('Telephone (include country code)', 'trx_addons') . '"></div>'
						. '</div>'
					. '</div>'
				...
				. '<div class="sc_contact_form_info_second">'

					. (!empty($subtitle) ? '<h4 class="sc_contact_subtitle">' . trim(themerex_strmacros($subtitle)) . '</h4>' : '')


					. '<div class="columns_wrap sc_columns columns_nofluid sc_columns_count_3">'
						. '<div class="column-1_3 sc_column_item sc_column_item_1 odd first">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_pieces">' . __('Number Of Pieces', 'trx_addons') . '</label><input id="sc_contact_form_pieces" type="text" name="pieces" placeholder="' . __('Number Of Pieces', 'trx_addons') . '"></div>'
						. '</div>'
						. '<div class="column-1_3 sc_column_item sc_column_item_2 even">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_weight">' . __('Total Weight (LBS)', 'trx_addons') . '</label><input id="sc_contact_form_weight" type="text" name="weight" placeholder="' . __('Total Weight (LBS)', 'trx_addons') . '"></div>'
						. '</div>'
					...
					. '<div class="columns_wrap sc_columns columns_nofluid sc_columns_count_3">'
						. '<div class="column-1_3 sc_column_item sc_column_item_1 odd first">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_height">' . __('Height (inches)', 'trx_addons') . '</label><input id="sc_contact_form_height" type="text" name="height" placeholder="' . __('Height (inches)', 'trx_addons') . '"></div>'
						. '</div>'
						. '<div class="column-1_3 sc_column_item sc_column_item_2 even">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_width">' . __('Width (inches)', 'trx_addons') . '</label><input id="sc_contact_form_width" type="text" name="width" placeholder="' . __('Width (inches)', 'trx_addons') . '"></div>'
						. '</div>'
						. '<div class="column-1_3 sc_column_item sc_column_item_3 odd">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_depth">' . __('Depth (inches)', 'trx_addons') . '</label><input id="sc_contact_form_depth" type="text" name="depth" placeholder="' . __('Depth (inches)', 'trx_addons') . '"></div>'
						. '</div>'
					. '</div>'
					...
					. '<div class="columns_wrap sc_columns columns_nofluid sc_columns_count_1">'
						. '<div class="column-1 sc_column_item sc_column_item_1 odd">'
							. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_commodity">' . __('Commodity', 'trx_addons') . '</label><input id="sc_contact_form_commodity" type="text" name="commodity" placeholder="' . __('Commodity', 'trx_addons') . '"></div>'
						. '</div>'
					. '</div>'

					. '<div class="sc_contact_form_item sc_contact_form_message label_over"><label class="required" for="sc_contact_form_message">' . __('Message Subject', 'trx_addons') . '</label><textarea id="sc_contact_form_message" name="message" placeholder="' . __('Message Subject', 'trx_addons') . '"></textarea></div>'
                . ((!empty($privacy)) ? '<div class="sc_form_field sc_form_field_checkbox">
                  ...
                  <label for="i_agree_privacy_policy_sc_form_' . esc_attr($cnt) . '">' . $privacy . '</label></div>' : '')
					. '<div class="sc_contact_form_item sc_contact_form_button"><button ' . (!empty($privacy) ? ' disabled="disabled"' : '') . '>' . __('Submit', 'trx_addons') . '</button></div>'
		...
		$output = '<div '
			...
			. '<div class="sc_contact_info">'
				...
				. (!empty($subtitle) ? '<h6 class="sc_contact_form_subtitle">' . trim(themerex_strmacros($subtitle)) . '</h6>' : '')
				...
				. '<div class="sc_contact_form_address_wrap">'
					. '<div class="sc_contact_form_address_field">'
						. '<span class="sc_contact_form_address_data">'.trim($address_2).(!empty($address_2) && !empty($address_1) ? '<br/>' : '').$address_1.'</span>'
					. '</div>'
			...
			. '<div class="sc_contact_info_bottom">'
				. '<div class="sc_contact_form_address_wrap">'
					. '<div class="sc_contact_form_address_field">'
						. '<span class="sc_contact_form_address_label">'.__('tel.: ', 'trx_addons').'</span>'
						. '<span class="sc_contact_form_address_data">'.'<a href="tel:'. $phone .'">' . trim($phone) . '</a>' .(!empty($phone) && !empty($fax) ? ', ' : '').$fax.'</span>'
					. '</div>'

					. '<div class="sc_contact_form_address_field">'
						. '<span class="sc_contact_form_address_label">'.__('e-mail: ', 'trx_addons').'</span>'
						. '<span class="sc_contact_form_address_data">'.'<a href="mailto:'. $email .'">' . trim($email) . '</a>' .'</span>'
					. '</div>'
				. '</div>'
			. do_shortcode('[trx_socials size="small" style="color"][/trx_socials]')
			. '</div>'
			...
			. ($content != '' && themerex_sc_param_is_on($custom)
				...
				. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_email">' . __('E-mail', 'trx_addons') . '</label><input id="sc_contact_form_email" type="text" name="email" placeholder="' . __('Email', 'trx_addons') . '"></div>'
                ...
                . ((!empty($privacy)) ? '<div class="sc_form_field sc_form_field_checkbox">
                  <input type="checkbox" id="i_agree_privacy_policy_sc_form_' . esc_attr($cnt) . '" name="i_agree_privacy_policy" class="sc_form_privacy_checkbox" value="1">
                  <label for="i_agree_privacy_policy_sc_form_' . esc_attr($cnt) . '">' . $privacy . '</label></div>' : '')
                . '<div class="sc_contact_form_item sc_contact_form_button"><button ' . (!empty($privacy) ? ' disabled="disabled"' : '') . '>' . __('Send message', 'trx_addons') . '<span class="icon-mail-alt"></span></button></div>'
			)
			. '<div class="result sc_infobox"></div>'
			. '</form>'
			. '</div>'
...
if ( !function_exists( 'sc_contact_form_send' ) ) {
	function themerex_sc_contact_form_send() {
		...
		if (!($contact_email = themerex_get_theme_option('contact_email')) && !($contact_email = themerex_get_theme_option('admin_email'))) 
			$response['error'] = __('Unknown admin email!', 'trx_addons');
		else {
			...
			if ($type_order=='order') {
				$user_username	= themerex_strshort($post_data['username'],	500);
				$user_username2	= themerex_strshort($post_data['username2'],	500);
				$user_email	    = themerex_strshort($post_data['email'],	100);
				$user_company	= themerex_strshort($post_data['company'],	800);
				$user_address	= themerex_strshort($post_data['address'],	800);
				$user_city	= themerex_strshort($post_data['city'],	800);
				$user_zip	= themerex_strshort($post_data['zip'],	800);
				$user_phone	= themerex_strshort($post_data['phone'],	500);
				$user_pieces	= themerex_strshort($post_data['pieces'],	500);
				$user_weight	= themerex_strshort($post_data['weight'],	500);
				$user_height	= themerex_strshort($post_data['height'],	500);
				$user_width	= themerex_strshort($post_data['width'],	500);
				$user_depth	= themerex_strshort($post_data['depth'],	500);
				$user_commodity	= themerex_strshort($post_data['commodity'],	800);
				$user_msg	= themerex_strshort($post_data['message'],	themerex_get_theme_option('message_maxlength_contacts'));

				$subj = sprintf(__('Site %s - Contact form message from %s', 'trx_addons'), get_bloginfo('site_name'), $user_email);
				$msg = "\n".__('First Name:', 'trx_addons') .' '.esc_html($user_username)
					.  "\n".__('Last Name:', 'trx_addons') .' '.esc_html($user_username2)
				    .  "\n".__('E-mail:', 'trx_addons') .' '.esc_html($user_email)
					.  "\n".__('Company:', 'trx_addons') .' '.esc_html($user_company)
					.  "\n".__('Address:', 'trx_addons') .' '.esc_html($user_address)
					.  "\n".__('City:', 'trx_addons') .' '.esc_html($user_city)
					.  "\n".__('Zip:', 'trx_addons') .' '.esc_html($user_zip)
					.  "\n".__('Telephone:', 'trx_addons') .' '.esc_html($user_phone)
					.  "\n".__('Number Of Pieces:', 'trx_addons') .' '.esc_html($user_pieces)
					.  "\n".__('Total Weight (LBS):', 'trx_addons') .' '.esc_html($user_weight)
					.  "\n".__('Height (inches):', 'trx_addons') .' '.esc_html($user_height)
					.  "\n".__('Width (inches):', 'trx_addons') .' '.esc_html($user_width)
					.  "\n".__('Depth (inches):', 'trx_addons') .' '.esc_html($user_depth)
					.  "\n".__('Commodity:', 'trx_addons') .' '.esc_html($user_commodity)
					.  "\n".__('Message:', 'trx_addons').' '.esc_html($user_msg);
				...
				$msg = "\n".__('Name:', 'trx_addons')   .' '.esc_html($user_name)
					.  "\n".__('E-mail:', 'trx_addons') .' '.esc_html($user_email)
					.  "\n".__('Subject:', 'trx_addons').' '.esc_html($user_subj)
					.  "\n".__('Message:', 'trx_addons').' '.esc_html($user_msg);

			} else {
				$subj = sprintf(__('Site %s - Custom form data', 'trx_addons'), get_bloginfo('site_name'));
				$msg = '';
			...
			if (!@$mail($contact_email, $subj, $msg)) {
				$response['error'] = __('Error send message!', 'trx_addons');
			}
...
function themerex_sc_content($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
		...
		. (!themerex_sc_param_is_off($animation) ? ' data-animation="'.esc_attr(themerex_sc_get_animation_classes($animation)).'"' : '')
		. ($css!='' ? ' style="'.esc_attr($css).'"' : '').'>'
		. ($title && $title != '' ? '<h4 class="subtitle_container"'.($color && $color!='' && $color!='inherit' ? 'style="color:'.$color.';"' :'').'>'.$title.'</h4>' : '')
		. do_shortcode($content) 
		. '</div>';
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
		...
		. '>'
			. '<div class="sc_countdown_item sc_countdown_days">'
				...
				. '<span class="sc_countdown_label">'.__('Days', 'trx_addons').'</span>'
			. '</div>'
			...
			. '<div class="sc_countdown_item sc_countdown_hours">'
				...
				. '<span class="sc_countdown_label">'.__('Hours', 'trx_addons').'</span>'
			. '</div>'
			. ($style==2 ? '<div class="sc_countdown_separator">:</div>' : '')
			. '<div class="sc_countdown_item sc_countdown_minutes">'
				. '<span class="sc_countdown_digits"><span></span><span></span></span>'
				. '<span class="sc_countdown_label">'.__('Minutes', 'trx_addons').'</span>'
			. '</div>'
			. ($style==2 ? '<div class="sc_countdown_separator">:</div>' : '')
			. '<div class="sc_countdown_item sc_countdown_seconds">'
				. '<span class="sc_countdown_digits"><span></span><span></span></span>'
				. '<span class="sc_countdown_label">'.__('Seconds', 'trx_addons').'</span>'
			. '</div>'
...
function themerex_sc_emailer($atts, $content = null) {
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
			...
			. '<span class="title">'.__('Newsletter', 'trx_addons').'</span>'
			. '<input type="text" class="sc_emailer_input" name="email" value="" placeholder="'.__('Enter your Email here', 'trx_addons').'">'
			. '<button class="sc_emailer_button" title="'.__('Submit', 'trx_addons').'" data-group="'.($group ? $group : __('E-mailer subscription', 'trx_addons')).'" ' . (!empty($privacy) ? ' disabled="disabled"' : '') . '>'.__('Subscribe', 'trx_addons').'</button>'
            . ((!empty($privacy)) ? '<div class="sc_form_field sc_form_field_checkbox">
                  <input type="checkbox" id="i_agree_privacy_policy_sc_form_' . esc_attr($cnt) . '" name="i_agree_privacy_policy" class="sc_form_privacy_checkbox" value="1">
                  <label for="i_agree_privacy_policy_sc_form_' . esc_attr($cnt) . '">' . $privacy . '</label></div>' : '')
        . '</form>'
...
themerex_require_shortcode("trx_googlemap", "themerex_sc_googlemap");
	...
	themerex_require_shortcode('trx_googlemap_marker', 'themerex_sc_googlemap_marker');
...
function themerex_sc_icon($atts, $content=null){	
	...
	$css2 = ($font_weight != '' && $font_weight != 'inherit' ? 'font-weight:'. esc_attr($font_weight).';' : '')
		. ($font_size != '' ? 'font-size:' . esc_attr(themerex_prepare_css_value($font_size)) . '; line-height: ' . (!$bg_shape || themerex_sc_param_is_inherit($bg_shape) ? '1' : '1.35') . 'em;' : '')
		. ($color != '' ? 'color:'.esc_attr($color).';border-color:'.esc_attr($color).';' : '')
		. ($bg_color != '' ? 'background-color:'.esc_attr($bg_color).';border-color:'.esc_attr($bg_color).';' : '')
	;
	$output = $icon!='' 
		? ($link ? '<a href="'.esc_url($link).'"' : '<span') . ($id ? ' id="'.esc_attr($id).'"' : '')
			. ' class="sc_icon '.esc_attr($icon).''.($title && $title!='' ? ' icon_title_on' : '')
			...
			.'>'
			. ($title && $title!='' ? '<span class="icon_title">'.esc_attr($title).'</span>' : '')
			.($link ? '</a>' : '</span>')
		: '';
...
function themerex_sc_image($atts, $content=null){	
	...
	if ( (strlen($url) > 0) and ( intval($url) > 0 ) ) {
		$alt = get_post_meta(intval($url), '_wp_attachment_image_alt', true);
	}
...
themerex_require_shortcode('trx_parallax', 'themerex_sc_parallax');
...
themerex_require_shortcode('trx_price', 'themerex_sc_price');
...
themerex_require_shortcode('trx_price_block', 'themerex_sc_price_block');
...
function themerex_sc_price_block($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
		...
		. '</div>'
			...
			. (!empty($link_text) ? '<div class="sc_price_block_link">'.do_shortcode('[trx_button bg_style="'.($style == 1 ? 'custom' : 'custom').'" link="'.($link ? esc_url($link) : '#').'"]'.($link_text).'[/trx_button]').'</div>' : '')
		. '</div>';
		...
		. '>
						<div class="search_form_wrap">
							<form role="search" method="get" class="search_form" action="' . esc_url( home_url( '/' ) ) . '">
								<button type="submit" class="search_submit icon-search" title="' . ($state=='closed' ? __('Open search', 'trx_addons') : __('Start search', 'trx_addons')) . '"></button>
...
function themerex_sc_section($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
		...
		. (themerex_sc_param_is_on($stylization) ? '<div class="sc_section_wrap">' : '')
			...
			. ($title && $title != '' ? '<h4 class="subtitle_container">'.$title.'</h4>' : '')
	...
	$output = ($type!='arc' && ($type!='pie' || !themerex_sc_param_is_on($pie_compact)) && $title!='' ? '<h3 class="sc_skills_title">'.($title).'</h3>' : '')
			. '<div id="'.esc_attr($id).'"' 
				. ' class="sc_skills sc_skills_' . esc_attr($type) 
					. (($type=='bar' || $type=='bar2' || $type=='bar3') ? ' sc_skills_'.esc_attr($dir) : '')
				...
				. (($type=='bar' || $type=='bar2' || $type=='bar3') ? ' data-dir="'.esc_attr($dir).'"' : '')
			. '>'
...
function themerex_sc_skills_item($atts, $content=null) {
	...
	if (empty($bg_color)) $bg_color = '#ffcb4f';
	...
	if ($THEMEREX_GLOBALS['sc_skills_type'] == 'arc' || ($THEMEREX_GLOBALS['sc_skills_type'] == 'pie' && themerex_sc_param_is_on($THEMEREX_GLOBALS['sc_skills_pie_compact']))) {
		...
		$output .= ($THEMEREX_GLOBALS['sc_skills_columns'] > 0 ? '<div class="sc_skills_column column-1_'.esc_attr($THEMEREX_GLOBALS['sc_skills_columns']).'">' : '')
				. (($THEMEREX_GLOBALS['sc_skills_type']=='bar' || $THEMEREX_GLOBALS['sc_skills_type']=='bar2' || $THEMEREX_GLOBALS['sc_skills_type']!='bar3' && $THEMEREX_GLOBALS['sc_skills_type']!='pie_2')  && $THEMEREX_GLOBALS['sc_skills_dir']=='horizontal' ? $title_block : '')
				. '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
		...
		if (in_array($THEMEREX_GLOBALS['sc_skills_type'], array('bar', 'counter'))) {
			$output .= '<div class="sc_skills_count"' . (($THEMEREX_GLOBALS['sc_skills_type']=='bar' || $THEMEREX_GLOBALS['sc_skills_type']=='bar2' || $THEMEREX_GLOBALS['sc_skills_type']=='bar3') && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
						...
						.'</div>'
					. '</div>';
		} else if (in_array($THEMEREX_GLOBALS['sc_skills_type'], array('bar3'))) {
			$output .=

				($icon!='' && $icon!='none' && $icon != 'inherit' ? ('<div class="sc_skills_icon"><span ' .($color_icon!='' && $color_icon!='none' ? 'style="color:'.$color_icon.'"' : ''). ' class="'.esc_attr($icon).'"></span></div>' ) :'')

				. '<div class="sc_skills_total'.($icon!='' && $icon!='none' && $icon != 'inherit' ? ' width_icon' :'').'"'
					. ' data-start="'.esc_attr($start).'"'
					. ' data-stop="'.esc_attr($stop).'"'
					. ' data-step="'.esc_attr($step).'"'
					. ' data-max="'.esc_attr($THEMEREX_GLOBALS['sc_skills_max']).'"'
				...
				.'<div class="sc_skills_count"' . ($color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
				. '</div>'
...
function themerex_sc_tabs($atts, $content = null) {
	...
	if($style == 2) {
		...
		$tabs_output =
			...
			.($subtitle && $subtitle!='' ? '<h4 class="subtitle">'.$subtitle.'</h4>' : '')
			.($title && $title!='' ? '<h2 class="title">'.$title.'</h2>' : '')
		...
		for ($i = 0; $i < count($sc_tab_titles); $i++) {
			...
			$titles_output .= '<li class="' . join(' ', $classes) . '">'
				. '<a href="#' . esc_attr($sc_tab_titles[$i]['id']) . '" class="theme_button" id="' . esc_attr($sc_tab_titles[$i]['id']) . '_tab">' . ($sc_tab_titles[$i]['title'])
		...
		$tabs_output =
		'<div class="sc_section sc_tabs_section">'
		...
		for ($i = 0; $i < count($sc_tab_titles); $i++) {
			...
			$titles_output .= '<li class="' . join(' ', $classes) . '">'
				. '<a href="#' . esc_attr($sc_tab_titles[$i]['id']) . '" class="theme_button" id="' . esc_attr($sc_tab_titles[$i]['id']) . '_tab">'

				. ($sc_tab_titles[$i]['icon'] && $sc_tab_titles[$i]['icon'] != 'inherit' ? '<span class="'.esc_attr($sc_tab_titles[$i]['icon']).'"></span>' : '')

				. ($sc_tab_titles[$i]['title'])
	...
	if (isset($sc_tab_titles[$THEMEREX_GLOBALS['sc_tab_counter']-1])) {
		...
		if (!empty($title)){
			$sc_tab_titles[$THEMEREX_GLOBALS['sc_tab_counter']-1]['title'] = $title;
				if (!empty($icon))
					$sc_tab_titles[$THEMEREX_GLOBALS['sc_tab_counter']-1]['icon'] = $icon;
		}
	} else {
		$sc_tab_titles[] = array(
			'id' => $id,
...
function themerex_sc_testimonials($atts, $content=null){	
	...
	if (themerex_sc_param_is_on($custom) && $content) {
		...
		while ( $query->have_posts() ) { 
			...
			$field = $post_meta['additional_field'];
...
themerex_require_shortcode('trx_twitter', 'themerex_sc_twitter');
...
themerex_require_shortcode('trx_zoom', 'themerex_sc_zoom');
...
themerex_require_shortcode('trx_gallery', 'themerex_sc_gallery');
...
themerex_require_shortcode('trx_sidebar', 'sc_sidebar');
