...
class themerex_emailer {
	...
	function admin_menu_item() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			//add_theme_page(__('Emailer', 'trx_addons'), __('Emailer', 'trx_addons'), 'edit_theme_options', 'trx_emailer', array($this, 'build_page'));

			// In this case menu item is add in admin menu 'Tools'
			add_management_page(__('Emailer', 'trx_addons'), __('Emailer', 'trx_addons'), 'manage_options', 'trx_emailer', array($this, 'build_page'));
		}
	}
	...
	function load_scripts() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='trx_emailer') {
			wp_enqueue_style('trx-emailer-style', trx_addons_get_file_url('tools/emailer/emailer.css'), array(), null);
		}
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='trx_emailer') {
			wp_enqueue_script('jquery-ui-core', false, array('jquery'), null, true);
			wp_enqueue_script('jquery-ui-tabs', false, array('jquery', 'jquery-ui-core'), null, true);
			wp_enqueue_script('trx-emailer-script', trx_addons_get_file_url('tools/emailer/emailer.js'), array('jquery'), null, true);
		}
	}
	...
	function build_page() {
		...
		if ( isset($_POST['emailer_subject']) ) {
			do {
				...
				if (empty($subject)) {
					$this->error = __('Subject can not be empty! Operation canceled!', 'trx_addons');
					break;
				}
				$message = isset($_POST['emailer_message']) ? $_POST['emailer_message'] : '';
				if (empty($message)) {
					$this->error = __('Message can not be empty! Operation canceled!', 'trx_addons');
					break;
				}
				...
				if (count($subscribers)==0) {
					$this->error = __('Subscribers lists are empty! Operation canceled!', 'trx_addons');
					break;
				}
				...
				if (empty($sender_email)) {
					$this->error = __('Sender email is empty! Operation canceled!', 'trx_addons');
					break;
				}
				...
				if ($subscribers_update && $group!='none') {
					...
					$add_msg = __(' The subscriber list is updated', 'trx_addons');
				} else if ($subscribers_delete && $group!='none') {
					unset($this->subscribers[$group]);
					update_option('themerex_emailer_subscribers', $this->subscribers);
					$add_msg = __(' The subscriber list is cleared', 'trx_addons');
				}
				$this->success = __('E-Mail was send successfull!', 'trx_addons') . $add_msg;
			} while (false);
		}

		?>
		<div class="trx_emailer">
			<h2 class="trx_emailer_title"><?php _e('ThemeREX Emailer', 'trx_addons'); ?></h2>
			...
			<form id="trx_emailer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="trx_emailer_block">
					<fieldset class="trx_emailer_block_inner">
						<legend> <?php _e('Letter data', 'trx_addons'); ?> </legend>
						<div class="trx_emailer_fields">
							<div class="trx_emailer_field trx_emailer_subject">
								<label for="emailer_subject"><?php _e('Subject:', 'trx_addons'); ?></label>
							...
							<div class="trx_emailer_field trx_emailer_attachment">
								<label for="emailer_attachment"><?php _e('Attachment:', 'trx_addons'); ?></label>
				...
				<div class="trx_emailer_block">
					<fieldset class="trx_emailer_block_inner">
						<legend> <?php _e('Subscribers', 'trx_addons'); ?> </legend>
						<div class="trx_emailer_fields">
							<div class="trx_emailer_field trx_emailer_group">
								<label for="emailer_group"><?php _e('Select group:', 'trx_addons'); ?></label>
								<select name="emailer_group" id="emailer_group">
									<option value="none"<?php echo $group=='none' ? ' selected="selected"' : ''; ?>><?php _e('- Select group -', 'trx_addons'); ?></option>
									<?php
								...
								<input type="checkbox" name="emailer_subscribers_update" id="emailer_subscribers_update" value="1"<?php echo $subscribers_update ? ' checked="checked"' : ''; ?> /><label for="emailer_subscribers_update" class="inline" title="<?php _e('Update the subscribers list for selected group', 'trx_addons'); ?>"><?php _e('Update', 'trx_addons'); ?></label>
								<input type="checkbox" name="emailer_subscribers_clear" id="emailer_subscribers_clear" value="1"<?php echo $subscribers_clear ? ' checked="checked"' : ''; ?> /><label for="emailer_subscribers_clear" class="inline" title="<?php _e('Clear this group from not confirmed emails after send', 'trx_addons'); ?>"><?php _e('Clear', 'trx_addons'); ?></label>
								<input type="checkbox" name="emailer_subscribers_delete" id="emailer_subscribers_delete" value="1"<?php echo $subscribers_delete ? ' checked="checked"' : ''; ?> /><label for="emailer_subscribers_delete" class="inline" title="<?php _e('Delete this group after send', 'trx_addons'); ?>"><?php _e('Delete', 'trx_addons'); ?></label>
							</div>
							<div class="trx_emailer_field trx_emailer_subscribers2">
								<label for="emailer_subscribers" class="big"><?php _e('List of recipients:', 'trx_addons'); ?></label>
							...
							<div class="trx_emailer_field trx_emailer_sender_name">
								<label for="emailer_sender_name"><?php _e('Sender name:', 'trx_addons'); ?></label>
							...
							<div class="trx_emailer_field trx_emailer_sender_email">
								<label for="emailer_sender_email"><?php _e('Sender email:', 'trx_addons'); ?></label>
				...
				<div class="trx_emailer_buttons">
					<a href="#" id="trx_emailer_send"><?php echo _e('Send', 'trx_addons'); ?></a>
				</div>
	...
	function themerex_callback_emailer_submit() {
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email]))
				$response['error'] = __('E-mail address already in the subscribers list!', 'trx_addons');
			else {
				$subscribers[$group][$email] = md5(mt_rand());
				update_option('themerex_emailer_subscribers', $subscribers);
				$subj = sprintf(__('Site %s - Subscribe confirmation', 'trx_addons'), get_bloginfo('site_name'));
				...
				if (is_email($email) && !@$mail($email, $subj, nl2br($msg), $headers)) {
					$response['error'] = __('Error send message!', 'trx_addons');
				}
				remove_filter( 'wp_mail_content_type', 'themerex_set_html_content_type' );
			}
		} else
			$response['error'] = __('E-mail address is not valid!', 'trx_addons');
		...
		if (preg_match('/[\.\-_A-Za-z0-9]+?@[\.\-A-Za-z0-9]+?[\ .A-Za-z0-9]{2,}/', $email)) {
			...
			if (isset($subscribers[$group][$email])) {
				if ($subscribers[$group][$email] == $nonce) {
					...
					update_option('themerex_emailer_subscribers', $subscribers);
					themerex_set_system_message(__('Confirmation complete! E-mail address succefully added in the subscribers list!', 'trx_addons'), 'success');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				} else if ($subscribers[$group][$email] != '') {
					themerex_set_system_message(__('Bad confirmation code!', 'trx_addons'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				} else {
					themerex_set_system_message(__('E-mail address already exists in the subscribers list!', 'trx_addons'), 'error');
					//header('Location: '.home_url());
					wp_safe_redirect( home_url() );
				}
			}
