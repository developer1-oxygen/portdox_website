...
if (!function_exists('trx_addons_is_on')) {
    function trx_addons_get_theme_option($prm) {
...
if ( !function_exists( 'trx_addons_enqueue_googlemap' ) ) {
	function trx_addons_enqueue_googlemap() {
		if (trx_addons_is_on(themerex_get_theme_option('api_google_load'))) {
...
if (!function_exists('trx_addons_get_folder_dir')) {	
	function trx_addons_get_folder_dir($folder, $return_url=false) {
        ...
        else if (is_dir(($TRX_ADDONS_STORAGE['plugin_dir']).($folder)))
            $dir = ($return_url ? $TRX_ADDONS_STORAGE['plugin_url'] : $TRX_ADDONS_STORAGE['plugin_dir']).($folder);
        return $dir;
	}
}
...
if (!function_exists('trx_addons_check_min_file')) {	
	function trx_addons_check_min_file($file, $dir) {
		if (substr($file, -3)=='.js') {
			if (substr($file, -7)!='.min.js' && trx_addons_is_off(themerex_get_theme_option('debug_mode', false, false))) {
			...
			if (substr($file, -8)!='.min.css'  && trx_addons_is_off(themerex_get_theme_option('debug_mode', false, false))) {
				$dir = trailingslashit($dir);
	...
	function trx_addons_fpc($file, $data, $flag=0) {
		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (trx_addons_is_on(themerex_get_theme_option('debug_mode', false, false)))
...
if (!function_exists('themerex_array_insert_before')) {
