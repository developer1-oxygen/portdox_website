...
if (!function_exists('themerex_add_fields_in_user_profile')) {
	...
	?>
		<h3><?php _e('User Position', 'trx_addons'); ?></h3>
		<table class="form-table">
			<tr>
				<th><label for="user_position"><?php _e('User position', 'trx_addons'); ?>:</label></th>
				<td><input type="text" name="user_position" id="user_position" size="55" value="<?php echo esc_attr(get_the_author_meta('user_position', $user->ID)); ?>" />
					<span class="description"><?php _e('Please, enter your position in the company', 'trx_addons'); ?></span>
				</td>
			</tr>
		</table>
	
		<h3><?php _e('Social links', 'trx_addons'); ?></h3>
		<table class="form-table">
		<?php
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
