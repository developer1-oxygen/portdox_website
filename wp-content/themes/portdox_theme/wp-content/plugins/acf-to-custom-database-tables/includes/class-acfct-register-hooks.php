...
class Acf_ct_register_hooks
{
		...
		if ($change_list !== false && $change_list['should_update'] === true) {
			...
			<?php if ($validate['valid'] === true): ?>
                <div class="notice notice-success is-dismissible">
                    <h3><?php echo esc_html(ACF_CT_PLUGIN_NAME); ?></h3>
                    <p><strong>Table Name</strong>: <?php echo esc_html($wpdb->prefix . $customTableName); ?></p>
                    <table>
						<?php
						/**
                    ...
                    <p><a class="button button-primary button-large" href="<?php echo esc_url(ACF_CT_ADMIN_PAGE."&type=sql-view&acf_ct_post_id=".intval($_GET['post'])); ?>">Go to Manage Tables</a></p>
                </div>
			<?php
			/**
	...
	public function acf_ct_filter_pre_update_value($check, $value, $post_id, $field)
	{
		...
		if (is_admin() === true || $is_update_field_support_disabled === true) {
			return null;
		}
