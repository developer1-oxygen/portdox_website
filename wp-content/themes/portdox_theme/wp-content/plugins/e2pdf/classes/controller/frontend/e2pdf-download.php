        ...
        if ($uid && $entry->load_by_uid($uid)) {
            ...
            if ($entry->get_data('pdf')) {
                if (file_exists($entry->get_data('pdf')) && $this->helper->load('filter')->is_downloadable($entry->get_data('pdf'))) {
                    ...
                    do_action('e2pdf_controller_frontend_e2pdf_download_response', $this->download, $uid, $entry->get('entry'), $file);
                }
            ...
            do_action('e2pdf_controller_frontend_e2pdf_download', $this->download, $uid, $entry->get('entry'));
        }
