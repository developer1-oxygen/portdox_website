...
class Controller_E2pdf extends Helper_E2pdf_View {
    ...
    public function bulk_action() {
        ...
        if (defined('DISABLE_WP_CRON') && DISABLE_WP_CRON) {
            $this->add_notification('error', __('The bulk export requires WordPress Cron to be Enabled.'));
        }
    ...
    public function ajax_templates() {
        ...
        if ($template->load($template_id)) {
            ...
            if ($template->extension()->method('delete_items')) {
        ...
        if ($template->load($template_id)) {
            foreach ($datasets as $key => $dataset_id) {
                ...
                if ($dataset) {
                    ...
                    if (isset($dataset->url) && $dataset->url) {
                        $content['datasets'][$key]['actions'][] = sprintf('<a target="_blank" class="e2pdf-link" href="%s">%s</a>', $dataset->url, __('View Dataset', 'e2pdf'));
                    }
                    if ($template->extension()->method('delete_item')) {
        ...
        if ($this->helper->get('license')->get('type') == 'FREE') {
            ...
            if ($template->load($template_id)) {
                ...
                if (!empty($datasets)) {
                    ...
                    if (!wp_next_scheduled('e2pdf_cronjob')) {
                        wp_schedule_event(time(), 'e2pdf_interval', 'e2pdf_cronjob');
                    }
