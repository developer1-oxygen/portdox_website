...
class Model_E2pdf_Options extends Model_E2pdf_Model {
    ...
    public static function get_options($all = true, $only_group = array(), $exclude = false) {
            ...
            'common_group' => array(
                ...
                'options' => array(
                    ...
                    array(
                        ...
                        'options' => array(
                            __('Stable', 'e2pdf'),
                            __('Release Candidate (For Beta Testers ONLY)', 'e2pdf')
                        )
                    ),
                    array(
                    ...
                    ,
                    array(
                        'name' => __('PDF User Key', 'e2pdf'),
                        'key' => 'pdf_user_key',
                        'value' => get_option('pdf_user_key') === false ? '' : get_option('pdf_user_key'),
                        'default_value' => '',
                        'type' => 'textarea',
                        'placeholder' => __('PDF User Key', 'e2pdf'),
                    )
                     ,
                    array(
                        'name' => __('PDF Product Key', 'e2pdf'),
                        'key' => 'pdf_product_key',
                        'value' => get_option('pdf_product_key') === false ? '' : get_option('pdf_product_key'),
                        'default_value' => '',
                        'type' => 'textarea',
                        'placeholder' => __('PDF Product Key', 'e2pdf'),
                    )
        ...
        if (class_exists('TRP_Translation_Render')) {
            if (get_option('e2pdf_translatepress_translation') !== false) {
                update_option('e2pdf_pdf_translation', get_option('e2pdf_translatepress_translation'));
            }
        }

        if (class_exists('TRP_Translation_Render') || class_exists('WeglotWP\Services\Translate_Service_Weglot')) {
