...
class Model_E2pdf_Extension extends Model_E2pdf_Model {
    ...
    public function render($value, $field = array()) {
        if ($this->extension() && method_exists($this->extension(), 'render')) {
            return $this->helper->load('translator')->translate($this->extension()->render($value, $field), 'full');
        }
        return false;
    }
    ...
    public function dataset($dataset_id = false) {
        if ($this->extension() && method_exists($this->extension(), 'dataset')) {
