...
class Extension_E2pdf_Wordpress extends Model_E2pdf_Model {
    ...
    public function search_strings($original_string, $different_strings) {
      foreach ($different_strings as $string) {
        if (strpos($original_string, $string) !== false) {
          return true;
        }
      }
      return false;
                ...
                if (!empty($tagnames)) {
                    ...
                    foreach ($shortcodes[0] as $key => $shortcode_value) {
                        ...
                        (
                            [0] => Array
                                (
                                    [0] => [e2pdf-wp key="ultimate_consignee" meta="true"]
                                )
                            ...
                            [3] => Array
                                (
                                    [0] =>  key="ultimate_consignee" meta="true"
                                )
                        ...
                        */
                        ...
                        if ($shortcode[2] === 'e2pdf-content') {
                            ...
                            if (substr($shortcode_value, -11) === '[/e2pdf-wp]' || substr($shortcode_value, -8) === '[/terms]' || substr($shortcode_value, -7) === '[/meta]') {
                                ...
                                $mystring           =   $shortcode['3'];

                                //$pdf_user_key       =   get_post_meta($post_id,  "pdf_user_key" , true);
                                //$pdf_product_key    =   get_post_meta($post_id,  "pdf_product_key" , true);
                                $pdf_user_key       = 'ocean_export_booking_freight_forwarder,ocean_export_booking_intermediate_consignee,ocean_export_booking_ultimate_consignee';
                                $pdf_product_key    = 'ocean_export_booking_selected_cars'; 


                                $findme             =   explode(",",$pdf_user_key);
                                
                                $findme2            =   explode(",",$pdf_product_key);

                                if ($this->search_strings($mystring, $findme))
                                {   

                                    /*   $myfile = fopen("/home/457959.cloudwaysapps.com/fwkcspffuz/public_html/3.html", "w") or die("Unable to open file!");
                                    ...
                                    */
                                    ...
                                    $string = $shortcode_value;

                                    preg_match('/key="(.*?)"\s+meta="(.*?)"/', $string, $matches);
                                    
                                    $meta_key22 = $matches[1];

                                    $user_id22=get_post_meta($post_id,  $meta_key22 , true);
                                 



                                    $first_name = get_user_meta($user_id22, 'first_name', true);
                                    $last_name = get_user_meta($user_id22, 'last_name', true);
                                    $address = get_user_meta($user_id22, 'address', true);
                                    $full_str = $first_name." ".$last_name. " \n".$address ;
                                    $value = str_replace($shortcode_value, "aaa ".$full_str, $value);
                                }
                                else if ($this->search_strings($mystring, $findme2))
                                {  
                                    ...
                                    $product_post   =   get_post_meta($post_id,  $meta_key22 , true);
                                   
                                    $cars_list      =   explode(",",$product_post);
                                    ob_start();
                                    ...
                                    ?>
                                     ...
                                     <table  >
                                            ...
                                            if(count($cars_list)) {
                                                ...
                                                foreach($cars_list as $product_id) 
                                                {
                                                  
                                                    $maker_and_number   =  get_field('vin',$product_id);
                                                    $number_of_package  =  get_field('eei_quantityone',$product_id)." ";
                                                    $description        =  get_field('description',$product_id);
                                                    $gross_weight       =  get_field('weight',$product_id);
                                                    $measurement        = 'na';
                                                    ...
                                                    ?>
                                                     <tr> 
                                                        <td style="width: 18%;"><?php echo $maker_and_number;?></td> 
                                                        <td style="width: 11%;"> <?php echo $number_of_package;?> </td> 
                                                        <td style="width: 45%;"><?php echo $description;?></td> 
                                                        <td style="width: 14%;"><?php echo $gross_weight;?></td>
                                                        <td style="width: 13%;"><?php echo $measurement;?></td> 
                                                    </tr>
                                                    <?php
                                                }
                                            }
                                    ...
                                    $value = str_replace($shortcode_value, $str, $value);
    ...
    public function visual_mapper() {
        ...
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Avatar", "e2pdf"), 'e2pdf-user key="user_avatar"')}</div>";
        $vc .= "</div>";
