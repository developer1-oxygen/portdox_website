...
class Extension_E2pdf_Woocommerce extends Model_E2pdf_Model {
    ...
    public function load_actions() {
        ...
        if (get_option('e2pdf_wc_cart_template_id')) {
            $e2pdf_wc_cart_template_id_order = '10';
            if (get_option('e2pdf_wc_cart_template_id_order') !== false) {
                $e2pdf_wc_cart_template_id_order = (int) get_option('e2pdf_wc_cart_template_id_order');
            }
            add_action('woocommerce_proceed_to_checkout', array($this, 'action_e2pdf_wc_cart_template_id'), $e2pdf_wc_cart_template_id_order);
        }

        if (get_option('e2pdf_wc_checkout_template_id')) {
            $e2pdf_wc_checkout_template_id_order = '10';
            if (get_option('e2pdf_wc_checkout_template_id_order') !== false) {
                $e2pdf_wc_checkout_template_id_order = (int) get_option('e2pdf_wc_checkout_template_id_order');
            }
            $e2pdf_wc_checkout_template_id_hook = 'woocommerce_review_order_before_submit';
            if (get_option('e2pdf_wc_checkout_template_id_hook') == 'woocommerce_review_order_after_submit') {
                $e2pdf_wc_checkout_template_id_hook = 'woocommerce_review_order_after_submit';
            }
            add_action($e2pdf_wc_checkout_template_id_hook, array($this, 'action_e2pdf_wc_checkout_template_id'), $e2pdf_wc_checkout_template_id_order);
        }
    ...
    public function action_e2pdf_wc_cart_template_id() {
        ...
        echo do_shortcode('[e2pdf-download id="' . get_option('e2pdf_wc_cart_template_id') . '" dataset="' . wc_get_page_id('cart') . '" class="button e2pdf-wc-download-button e2pdf-wc-download-cart-button"]');
    }
    ...
    public function action_e2pdf_wc_checkout_template_id() {
        ...
        echo do_shortcode('[e2pdf-download id="' . get_option('e2pdf_wc_cart_template_id') . '" dataset="' . wc_get_page_id('cart') . '" class="button e2pdf-wc-download-button e2pdf-wc-download-checkout-button"]');
    }
                                    ...
                                    if ($_gravity_forms_history && !empty($_gravity_forms_history['_gravity_form_linked_entry_id'])) {
                                        ...
                                        if ($attachment) {
                                            ...
                                            if (($item_shortcode[2] === 'e2pdf-save' && isset($atts['attachment']) && $atts['attachment'] == 'true') || $item_shortcode[2] === 'e2pdf-attachment') {
                                                ...
                                                if ($file) {
                                                    if ($item_shortcode[2] != 'e2pdf-save' && !isset($atts['pdf'])) {
                                                        $this->helper->add('woocommerce_attachments', $file);
                                                    }
                                                    $attachments[] = $file;
                                                }
        ...
        if (!empty($downloads)) {
            ...
            foreach ($downloads as $key => $download) {
                if (isset($download['file']['file']) && $download['file']['file'] && strpos($download['file']['file'], '[e2pdf-download') !== false) {
                    ...
                    $item_id = $wpdb->get_var($wpdb->prepare("SELECT `items`.`order_item_id` FROM " . $wpdb->prefix . "woocommerce_order_items `items` INNER JOIN " . $wpdb->prefix . "woocommerce_order_itemmeta `itemmeta` ON `itemmeta`.`order_item_id` = `items`.`order_item_id` AND `itemmeta`.`meta_key` = '_product_id' AND `itemmeta`.`meta_value` = '" . $download['product_id'] . "' WHERE `items`.`order_id` = '" . $download['order_id'] . "' and `items`.`order_item_type` = 'line_item' and `items`.`order_item_id` NOT IN ( '" . implode("','", $item_ids) . "' ) ORDER BY `items`.`order_item_id` ASC"));
                    if ($item_id) {
    ...
    public function filter_e2pdf_model_options_get_options_options($options = array()) {
        ...
        $invoice_statuses = !is_array(get_option('e2pdf_wc_invoice_statuses')) ? array() : get_option('e2pdf_wc_invoice_statuses');
        ...
        foreach ($order_statuses as $order_status_key => $order_status) {
            $invoice_statuses_options[] = array(
                'name' => $order_status,
                'key' => 'e2pdf_wc_invoice_statuses[]',
                'value' => in_array($order_status_key, $invoice_statuses) ? $order_status_key : '',
        ...
        $invoice_statuses_options[] = array(
            'name' => $order_status,
            'key' => 'e2pdf_wc_invoice_statuses[]',
            'value' => in_array('any', $invoice_statuses) || get_option('e2pdf_wc_invoice_statuses') === false ? 'any' : '',
            'checkbox_value' => 'any',
        ...
        $options['woocommerce_group'] = array(
            ...
            'options' => array(
                ...
                array(
                    'name' => __('Status to Show Invoice', 'e2pdf'),
                    'key' => 'e2pdf_wc_invoice_statuses',
                    'type' => 'checkbox_list',
                    'options' => $invoice_statuses_options,
                ),
                array(
                    'name' => __('Cart PDF Button Template', 'e2pdf'),
                ...
                array(
                    'name' => __('Cart PDF Button Order', 'e2pdf'),
                    'key' => 'e2pdf_wc_cart_template_id_order',
                    'value' => get_option('e2pdf_wc_cart_template_id_order') === false ? '10' : get_option('e2pdf_wc_cart_template_id_order'),
                    'default_value' => '10',
                    'type' => 'text',
                    'class' => 'e2pdf-numbers',
                    'placeholder' => '0'
                ...
                array(
                    'name' => __('Checkout PDF Button Template', 'e2pdf'),
                ...
                array(
                    'name' => __('Checkout PDF Button Hook', 'e2pdf'),
                ...
                array(
                    'name' => __('Checkout PDF Button Order', 'e2pdf'),
                    'key' => 'e2pdf_wc_checkout_template_id_order',
                    'value' => get_option('e2pdf_wc_checkout_template_id_order') === false ? '10' : get_option('e2pdf_wc_checkout_template_id_order'),
                    'default_value' => '10',
                    'type' => 'text',
                    'class' => 'e2pdf-numbers',
                    'placeholder' => '0'
    ...
    function filter_woocommerce_my_account_my_orders_actions($actions, $order) {
        $invoice_statuses = !is_array(get_option('e2pdf_wc_invoice_statuses')) ? array() : get_option('e2pdf_wc_invoice_statuses');
        if (in_array($order->get_status(), $invoice_statuses) || in_array("wc-" . $order->get_status(), $invoice_statuses) || in_array('any', $invoice_statuses) || get_option('e2pdf_wc_invoice_statuses') === false) {
            $actions['e2pdf_invoice'] = array(
                ...
                'name' => apply_filters('e2pdf_wc_my_account_my_orders_actions_invoice_title', do_shortcode('[e2pdf-download id="' . get_option('e2pdf_wc_invoice_template_id') . '" dataset="' . $order->get_id() . '" output="button_title"]'))
            );
        }
        return apply_filters('e2pdf_wc_filter_woocommerce_my_account_my_orders_actions', $actions, $order);
    }

    function filter_woocommerce_product_downloads_approved_directory_validation_for_shortcodes($enabled) {
        if (get_option('wc_downloads_approved_directories_mode') === 'enabled') {
            return false;
        }
        return $enabled;
    ...
    public function visual_mapper() {
        ...
        if ($this->get('item') == 'cart') {
            ...
            $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Vatar URL", "e2pdf"), 'e2pdf-wc-customer key="get_avatar_url"')}</div>";
    ...
    private function get_vm_product() {
        ...
        $vc .= "<div class='e2pdf-grid'>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("ID", "e2pdf"), 'e2pdf-wc-product key="id"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Author", "e2pdf"), 'e2pdf-wc-product key="post_author"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Date", "e2pdf"), 'e2pdf-wc-product key="post_date"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Date (GMT)", "e2pdf"), 'e2pdf-wc-product key="post_date_gmt"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Content", "e2pdf"), 'e2pdf-wc-product key="post_content"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Title", "e2pdf"), 'e2pdf-wc-product key="post_title"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Excerpt", "e2pdf"), 'e2pdf-wc-product key="post_excerpt"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Status", "e2pdf"), 'e2pdf-wc-product key="post_status"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Comment Status", "e2pdf"), 'e2pdf-wc-product key="comment_status"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Ping Status", "e2pdf"), 'e2pdf-wc-product key="ping_status"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Password", "e2pdf"), 'e2pdf-wc-product key="post_password"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Name", "e2pdf"), 'e2pdf-wc-product key="post_name"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("To Ping", "e2pdf"), 'e2pdf-wc-product key="to_ping"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Ping", "e2pdf"), 'e2pdf-wc-product key="pinged"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Modified Date", "e2pdf"), 'e2pdf-wc-product key="post_modified"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Modified Date (GMT)", "e2pdf"), 'e2pdf-wc-product key="post_modified_gmt"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Filtered Content", "e2pdf"), 'e2pdf-wc-product key="post_content_filtered"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Parent ID", "e2pdf"), 'e2pdf-wc-product key="post_parent"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("GUID", "e2pdf"), 'e2pdf-wc-product key="guid"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Menu Order", "e2pdf"), 'e2pdf-wc-product key="menu_order"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Type", "e2pdf"), 'e2pdf-wc-product key="post_type"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Mime Type", "e2pdf"), 'e2pdf-wc-product key="post_mime_type"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Comments Count", "e2pdf"), 'e2pdf-wc-product key="comment_count"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Filter", "e2pdf"), 'e2pdf-wc-product key="filter"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Post Thumbnail", "e2pdf"), 'e2pdf-wc-product key="get_the_post_thumbnail"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Post Thumbnail URL", "e2pdf"), 'e2pdf-wc-product key="get_the_post_thumbnail_url"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Permalink", "e2pdf"), 'e2pdf-wc-product key="permalink"')}</div>";
        $vc .= "<div class='e2pdf-ib e2pdf-w50 e2pdf-vm-item'>{$this->get_vm_element(__("Post Permalink", "e2pdf"), 'e2pdf-wc-product key="get_post_permalink"')}</div>";
        $vc .= "</div>";
