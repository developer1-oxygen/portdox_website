...
<!-- 	<div class="filters -f<?php echo esc_attr($filter_count); ?>">
    ...
    jQuery(document).ready(function($) {
        ...
        $this.select2({
          ajax: {
            url: '<?php echo site_url();?>/wp-admin/admin-ajax.php?_fs_blog_admin=true',
            ...
            data: function(params) {
              return {
                ...
                field_key:$this.attr('field_key'),
                nonce: '<?php echo esc_attr($field['ajax_nonce']); ?>',
         ...
         <?php echo ($field['max']>1)?' , maximumSelectionLength : "1111" ' : '';?>
          
        });

        //alert('<?php echo $field['max'];?>');
     
    });
    </script>

    <?php #print_r($field['label']);?>
    <!---data-maximum-selection-length="<?php echo ($field['max']=="")?1000000:$field['max'];?>" min="<?php echo $field['min'];?>" --->

    <?php $multiple_str =  ($field['max']>1)?' multiple="multiple" ':''; ?>

	<div class="selection">
		<?php  #echo "<pre>"; print_r($field); echo "</pre>";?>
	  <select data-label="<?php echo $field['label'];?>"  name="<?php echo esc_attr( $field['name'] ); ?>[]" class="<?php echo $field['key'];?>"  class="acf-relationship-field acf-input select2-ajax" post_type="<?php echo implode(",",$field['post_type']);?>" field_key="<?php echo $field['key'];?>" data-label="<?php echo $field['label'];?>" tags="false" <?php echo $multiple_str;?>
	    >
	    <?php if( !empty($field['value']) ):
	        // get posts
	        $posts = acf_get_posts(array(
	        ...
	        // loop
	        foreach( $posts as $post ): ?>
	            <option value="<?php echo esc_attr($post->ID); ?>" selected><?php echo acf_esc_html( $this->get_post_title( $post, $field ) ); ?></option>
	        <?php endforeach; ?>
	    <?php endif; ?>
	  </select>
	</div>
...
<!-- 
	<div class="selection">
		...
		<div class="values">
			...
			<select name="<?php echo esc_attr($field['name']); ?>[]" multiple="multiple" data-placeholder="<?php echo esc_attr($field['placeholder']); ?>" data-minimum-input-length="<?php echo esc_attr($field['minimum_input_length']); ?>" data-ajax-url="<?php echo esc_url($field['ajax_action']); ?>" data-ajax-nonce="<?php echo esc_attr($field['ajax_nonce']); ?>" field_key="<?php  echo $field['key']?>"  post_id="<?php  echo $post->ID; ?>" >
				<?php if (!empty($field['value'])): 
					// get posts
					$posts = acf_get_posts(array(
						'post__in' => $field['value'],
