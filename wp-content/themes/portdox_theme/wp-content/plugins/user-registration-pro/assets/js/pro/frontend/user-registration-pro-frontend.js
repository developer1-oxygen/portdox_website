...
(function ($) {
		...
		$('.input-wrapper').each(function() {
			if ( $(this).children().length == 2 ) {
				...
				if ( firstEl.hasClass('password-input-group') ) {
					firstEl.children(":first").addClass('ur-pro-input-icon-padding');
				} else {
	...
	function enable_keyboard_friendly_form() {
		if( 'yes' === user_registration_pro_frontend_data.keyboard_friendly_form_enabled ||
			'1' === user_registration_pro_frontend_data.keyboard_friendly_form_enabled ) {
			...
			$('body').on("keydown", function (e) {
				...
				if(ur_fields.last().is(':focus')) {
					if( 9 === e.which ) {
						e.preventDefault();
						$( 'form.register' ).find('.ur-submit-button').first().trigger('focus');
					}
				}else if($( 'form.register' ).find('.ur-submit-button.button').is(':focus')) {
					if( 9 === e.which ) {
						e.preventDefault();
						ur_fields.first().trigger('focus');
					}
