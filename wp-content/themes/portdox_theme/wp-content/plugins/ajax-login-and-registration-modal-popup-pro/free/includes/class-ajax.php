...
function get_list_of_company_new($user_id22)
{
    $role = 'company';
    $args = array(
        'role' => $role,
...
class LRM_AJAX
{
    ...
    public static function login() {
        ...
        $user22 = get_userdata( $user_id );
        $user_roles22 = $user22->roles;
       
        if ( in_array( 'employee_11', $user_roles22, true ) or in_array( 'employee_22', $user_roles22, true )  or in_array( 'employee_33', $user_roles22, true ) ) {
            $comp_ids = get_list_of_company_new($user_id);
            $comp_id = @$_POST['comp_id'];
          
            if(!in_array($comp_id,$comp_ids))
            {
               
                echo '{"success":false,"data":{"message":"Company code not valid","for":"comp_id"}}';
                exit;
            }
            else
            {
        ...
        if ( in_array( 'company', $user_roles22, true ) ) {
            ...
            if($user_id!=$comp_id)
            {
                ...
                echo '{"success":false,"data":{"message":"Company code not valid22","for":"comp_id"}}';
                exit;
            }
            else
            {
