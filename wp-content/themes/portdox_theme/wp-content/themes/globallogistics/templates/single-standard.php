...
if ( !function_exists( 'themerex_template_single_standard_theme_setup' ) ) {
	...
	function themerex_template_single_standard_theme_setup() {
		themerex_add_template(array(
			...
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single standard', 'globallogistics'),
			'thumb_title'  => esc_html__('Fullwidth image', 'globallogistics'),
			'w'		 => 1150,
			'h'		 => 647
		));
	}
...
if ( !function_exists( 'themerex_template_single_standard_output' ) ) {
	function themerex_template_single_standard_output($post_options, $post_data) {
		...
		if ($post_data['post_protected']) {
			...
			if (function_exists('themerex_sc_reviews_placeholder') && themerex_strpos($post_data['post_content'], themerex_sc_reviews_placeholder())===false) {
			    if (shortcode_exists('trx_reviews')) {
                    $post_data['post_content'] = do_shortcode('[trx_reviews]') . ($post_data['post_content']);
                } else {
                    $post_data['post_content'] = themerex_sc_reviews(array()) . ($post_data['post_content']);
                }
            }
