...
if ( !function_exists( 'themerex_template_single_fullscreen_theme_setup' ) ) {
	add_action( 'themerex_action_before_init_theme', 'themerex_template_single_fullscreen_theme_setup', 1 );
	function themerex_template_single_fullscreen_theme_setup() {
		themerex_add_template(array(
			'layout' => 'single-fullscreen',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single fullscreen', 'globallogistics'),
			'thumb_title'  => esc_html__('Fullscreen image', 'globallogistics'),
			'w'		 => 1920,
			'h'		 => 1080
		));
	}
...
// Template output
if ( !function_exists( 'themerex_template_single_fullscreen_output' ) ) {
	function themerex_template_single_fullscreen_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] == 'center' && (themerex_get_custom_option('show_page_top')=='no' || themerex_get_custom_option('show_page_title')=='no')) {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="<?php themerex_show_layout($avg_author > 0 || $avg_users > 0 ? 'itemReviewed' : 'name'); ?>" class="post_title entry-title"><?php themerex_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
		<?php
		}
		...
		if ($post_data['post_protected']) {
			...
			if (function_exists('themerex_sc_reviews_placeholder') && themerex_strpos($post_data['post_content'], themerex_sc_reviews_placeholder())===false) $post_data['post_content'] = do_shortcode('[trx_reviews]') . ($post_data['post_content']);
            if(function_exists('themerex_sc_gap_wrapper')){
                themerex_show_layout(themerex_sc_gap_wrapper(themerex_sc_reviews_wrapper($post_data['post_content'])));
            } else {
			...
			if ( themerex_get_custom_option('show_post_tags') == 'yes' && (!empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links) || !empty($post_data['post_terms'][$post_data['post_taxonomy']]->terms_links))) {
				...
				<div class="post_info post_info_bottom">
					...
					<span class="post_info_item post_info_tags"><?php esc_html_e('Tags: ', 'globallogistics'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
					<?php }  ?>
					<?php if (!empty($post_data['post_terms'][$post_data['post_taxonomy']]->terms_links)) { ?>
						<span class="post_info_item post_info_category"><?php esc_html_e('Category: ', 'globallogistics'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy']]->terms_links); ?></span>
					<?php
					} ?>
