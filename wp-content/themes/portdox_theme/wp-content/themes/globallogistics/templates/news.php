...
if ( !function_exists( 'themerex_template_news_theme_setup' ) ) {
	...
	function themerex_template_news_theme_setup() {
		themerex_add_template(array(
			...
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: News', 'globallogistics'),
			'thumb_title'  => esc_html__('Medium image NEW (crop)', 'globallogistics'),
			'w'		 => 400,
			'h'		 => 250
		));
	}
...
if ( !function_exists( 'themerex_template_news_output' ) ) {
	function themerex_template_news_output($post_options, $post_data) {
		...
		$title_tag = $post_options['columns_count'] > 3 ? 'h4' : 'h3';
		...
		if (themerex_sc_param_is_on($post_options['scroll']) || ($post_options['dir'] == 'horizontal' && $post_options['columns_count'] > 0)) {
			...
			<div class="<?php echo 'column-1_'.esc_attr($post_options['columns_count']).' column_item_'.esc_attr($post_options['number']); ?><?php 
				themerex_show_layout(($post_options['number'] % 2 == 1 ? ' odd' : ' even')
		...
		<div class="post_item post_item_news sc_blogger_item<?php themerex_show_layout($post_options['number'] == $post_options['posts_on_page'] && !themerex_sc_param_is_on($post_options['loadmore']) ? ' sc_blogger_item_last' : '');
			?>">
			<?php 
				...
				if ($post_options['descr'] > 0) {
					...
					<?php
						if ($post_data['post_protected']) {
							...
							if (empty($post_options['readmore'])) $post_options['readmore'] = esc_html__('Learn more', 'globallogistics');
							if (!themerex_param_is_off($post_options['readmore']) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
								echo do_shortcode('[trx_button icon="icon-right" bg_style="underline" link="'.esc_url($post_data['post_link']).'"]'.($post_options['readmore']).'[/trx_button]');
							}
						}
					?>
					</div>
