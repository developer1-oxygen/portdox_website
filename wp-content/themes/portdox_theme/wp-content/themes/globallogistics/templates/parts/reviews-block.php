...
if ($avg_author > 0 || $avg_users > 0) {
	...
	$output = $marks = $users = '';
	if ($use_tabs) {
		$author_tab = '<li class="sc_tabs_title"><a href="#author_marks" class="theme_button">'.esc_html__('Author', 'globallogistics').'</a></li>';
		$users_tab = '<li class="sc_tabs_title"><a href="#users_marks" class="theme_button">'.esc_html__('Users', 'globallogistics').'</a></li>';
	...
	if (!$reviews_first_author || !$reviews_second_hide) {
		...
		$field["descr"] = wp_kses_data(sprintf(__("Summary rating from <b>%s</b> user's marks.", 'globallogistics'), $users)) . ' '
			.(!isset($_COOKIE['themerex_votes']) || themerex_strpos($_COOKIE['themerex_votes'], ','.($post_data['post_id']).',')===false
				? esc_html__('You can set own marks for this article - just click on stars above and press "Accept".', 'globallogistics')
				: esc_html__('Thanks for your vote!', 'globallogistics'));
	...
	if ($allow_user_marks) {
        ...
        themerex_set_global_array('js_vars', 'reviews_vote', isset($_COOKIE['themerex_votes']) ? $_COOKIE['themerex_votes'] : '');
