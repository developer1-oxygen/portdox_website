...
?>
<label for="admin_roles2">Assign Task to:</label>
<select id="admin_roles2" name="admin_roles">
...
<button id="assign_task_button2" type="button" class="button">Assign Task</button>
<script>
jQuery(document).ready(function ($) {
    $('#assign_task_button2').on('click', function () {
        var userId = $('#admin_roles2').val();
        var postId = <?php echo esc_js($post->ID); ?>;
        if(userId=="")
        {
        	alert("Please select loader");
        	return false;
        }
        // AJAX request
        $.ajax({
...
<div id="assign_result2"></div>


<hr style="margin-top: 10px; margin-bottom:10px ">

<form id="upload-image-form2" enctype="multipart/form-data" style="padding: 10px; border: 1px dotted #ccc; border-radius: 5px; "> 
	<input type="file" id="image2" name="image2" accept="image/*" > 
	<button type="button" id="upload_btn2"> Upload </button>
</form>
<div id="progress_bar2"></div>

<div id="upload_content" style="margin-top:10px; overflow: hidden;">
	<?php  display_existing_images_loader(get_the_ID()); ?>
</div>
