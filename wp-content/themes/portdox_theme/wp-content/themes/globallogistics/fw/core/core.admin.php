	...
	if ( !function_exists( 'themerex_admin_prepare_scripts' ) ) {
		...
		function themerex_admin_prepare_scripts() {
            ...
            $vars = array_merge($vars, array(
                ...
                'msg_importer_full_alert' => esc_html__("ATTENTION!\n\nIn this case ALL THE OLD DATA WILL BE ERASED\nand YOU WILL GET A NEW SET OF POSTS, pages and menu items.", 'globallogistics')
                    . "\n\n"
                    . esc_html__("It is strongly recommended only for new installations of WordPress\n(without posts, pages and any other data)!", 'globallogistics')
                    . "\n\n"
                    . esc_html__("Press OK to continue or Cancel to return to a partial installation", 'globallogistics'),
                'user_logged_in' => true
            ));
            wp_localize_script('themerex-options-script', 'THEMEREX_GLOBALS', apply_filters('themerex_action_add_scripts_inline', $vars));
	...
	if ( !function_exists( 'themerex_callback_admin_change_post_type' ) ) {
		...
		function themerex_callback_admin_change_post_type() {
			if ( !wp_verify_nonce( $_REQUEST['nonce'], 'ajax_nonce' ) )
				wp_die();
			$post_type = themerex_get_value_gp('post_type');
			$terms = themerex_get_list_terms(false, themerex_get_taxonomy_categories_by_post_type($post_type));
			$terms = themerex_array_merge(array(0 => esc_html__('- Select category -', 'globallogistics')), $terms);
			$response = array(
				'error' => '',
				'data' => array(
		...
		function themerex_admin_register_plugins() {
			...
			$plugins = apply_filters('themerex_filter_required_plugins', array(
				array(
					...
					'version'   => '1.2',
					'source'	=> themerex_get_file_dir('plugins/universal_services_plugin.zip'),
					'required' 	=> true
				),
				array(
