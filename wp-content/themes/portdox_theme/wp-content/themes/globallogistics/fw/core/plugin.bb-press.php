...
if ( !function_exists( 'themerex_bbpress_detect_inheritance_key' ) ) {
	//Handler of add_filter('themerex_filter_detect_inheritance_key',	'themerex_bbpress_detect_inheritance_key', 9, 1);
...
if ( !function_exists( 'themerex_bbpress_get_blog_type' ) ) {
	//Handler of add_filter('themerex_filter_get_blog_type',	'themerex_bbpress_get_blog_type', 9, 2);
...
if ( !function_exists( 'themerex_bbpress_get_blog_title' ) ) {
	//Handler of add_filter('themerex_filter_get_blog_title',	'themerex_bbpress_get_blog_title', 9, 2);
	function themerex_bbpress_get_blog_title($title, $page) {
		...
		if ( themerex_strpos($page, 'bbpress')!==false ) {
			if ( $page == 'bbpress_forum' || $page == 'bbpress_topic' ) {
				...
				$title = esc_html__('Forums', 'globallogistics');
			}
		}
		return $title;
	}
...
if ( !function_exists( 'themerex_bbpress_get_stream_page_title' ) ) {
	//Handler of add_filter('themerex_filter_get_stream_page_title',	'themerex_bbpress_get_stream_page_title', 9, 2);
...
if ( !function_exists( 'themerex_bbpress_get_stream_page_id' ) ) {
	//Handler of add_filter('themerex_filter_get_stream_page_id',	'themerex_bbpress_get_stream_page_id', 9, 2);
...
if ( !function_exists( 'themerex_bbpress_get_stream_page_link' ) ) {
	//Handler of add_filter('themerex_filter_get_stream_page_link', 'themerex_bbpress_get_stream_page_link', 9, 2);
...
if ( !function_exists( 'themerex_bbpress_frontend_scripts' ) ) {
	//Handler of add_action( 'themerex_action_add_styles', 'themerex_bbpress_frontend_scripts' );
	function themerex_bbpress_frontend_scripts() {
		wp_enqueue_style( 'themerex-bbpress-style',  themerex_get_file_url('css/bbpress-style.css'), array(), null );
	}
}
?>