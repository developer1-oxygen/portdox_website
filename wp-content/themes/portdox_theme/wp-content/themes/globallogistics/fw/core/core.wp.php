...
if (!function_exists('themerex_show_breadcrumbs')) {
	function themerex_show_breadcrumbs($args=array()) {
		...
		$args = array_merge( array(
			'home' => esc_html__('Home', 'globallogistics'),		// Home page title (if empty - not showed)
		...
		if ($type == 'page' || $type == 'attachment' || is_single()) {
			$pageParentID = apply_filters('themerex_filter_get_parent_id', isset($wp_query->post->post_parent) ? $wp_query->post->post_parent : 0, isset($wp_query->post->ID) ? $wp_query->post->ID : 0);
			...
			while ($pageParentID > 0) {
				...
				if ($level > $args['max_levels'])
					...
					$rez_parent = '<a class="breadcrumbs_item cat_post" href="' . esc_url(get_permalink($pageParent->ID)) . '">'
									...
									. ($rez_parent);
				if (($pageParentID = apply_filters('themerex_filter_get_parent_id', $pageParent->post_parent, $pageParentID)) > 0) $post_id = $pageParentID;
			}
		}
							...
							if ($level > $args['max_levels'])
								...
								$rez_all .= (!empty($rez_all) ? $args['delimiter'] : '') 
										. '<a class="breadcrumbs_item all" href="' . esc_url(get_permalink($tpl_id)) . '">'
											. sprintf(esc_html__('All %s', 'globallogistics'), themerex_get_post_format_name($post_format, false)) 
										. '</a>';
						}
					}
					if ($taxonomy != 'category' || in_array(get_post_type($post_id), array('post', 'attachment'))) {
...
if (!function_exists('themerex_show_pagination')) {
	function themerex_show_pagination($args=array()) {
		$args = array_merge(array(
			...
			'first_text'=> esc_html__('&laquo; First', 'globallogistics'),
			'last_text' => esc_html__("Last &raquo;", 'globallogistics'),
			'prev_text' => esc_html__("&laquo; Prev", 'globallogistics'),
			'next_text' => esc_html__("Next &raquo;", 'globallogistics'),
			'dot_text' => "&hellip;",
			'before' => '',
			'after' => '',
			),  is_array($args) ? $args 
...
if (!function_exists('themerex_show_pagination_single')) {
	function themerex_show_pagination_single( $args ) {
		...
		if ( is_single() ) {
			?>
			<h3 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'globallogistics' ); ?></h3>
			<?php
			...
			if ($max_page > 1) {
				...
				if ($opt['style'] == 'pages') {
					...
					if (empty($opt['pages_text'])) 
						$opt['pages_text'] = esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'globallogistics');
...
if ( !function_exists( 'themerex_callback_view_more_posts' ) ) {
	function themerex_callback_view_more_posts() {
		...
		$page = themerex_get_value_gp('page');
		...
		if ($page > 0 && is_array($args) && is_array($vars)) {
			...
			$response['error'] = esc_html__('Wrong query arguments', 'globallogistics');
		}
...
if ( !function_exists( 'themerex_callback_ajax_search' ) ) {
	function themerex_callback_ajax_search() {
		...
		if (!empty($s)) {
			...
			if (empty($output)) {
				$output .= '<article class="post_item">' . esc_html__('Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'globallogistics') . '</article>';
			} else {
				$output .= '<article class="post_item"><a href="#" class="post_more">' . esc_html__('More results &hellip;', 'globallogistics') . '</a></article>';
			}
			$response['data'] = $output;
		} else {
			$response['error'] = esc_html__('The query string is empty!', 'globallogistics');
		}
...
if (!function_exists('themerex_get_calendar')) {
	function themerex_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		if ( isset($_GET['w']) ) $w = ''.intval(themerex_get_value_gp('w'));
		...
		$calendar_caption = _x('%1$s %2$s', 'calendar caption', 'globallogistics');
		...
		if ( $prev_year+$prev_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($prev_year) . '" data-month="' . esc_attr($prev_month) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'globallogistics'), $wp_locale->get_month($prev_month), date('Y', mktime(0, 0, 0, $prev_month, 1, $prev_year)))) . '">'
		...
		if (count($posts) > (!empty($posts['done']) ? 1 : 0)) {
			...
			$link = '';
		$calendar_output .= '<th class="month_cur" colspan="5">' . ($link ? '<a href="' . esc_url($link) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'globallogistics'), $wp_locale->get_month($thismonth), date('Y', mktime(0, 0, 0, $thismonth, 1, $thisyear)))) . '">' : '') . sprintf($calendar_caption, $wp_locale->get_month($thismonth), '<span>'.date('Y', $unixmonth).'</span>') . ($link ? '</a>' : '') . '</th>';
		...
		if ( $next_year+$next_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($next_year) . '" data-month="' . esc_attr($next_month) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'globallogistics'), $wp_locale->get_month($next_month), date('Y', mktime(0, 0 , 0, $next_month, 1, $next_year))) ) . '">'
		...
		for ( $day = 1; $day <= $daysinmonth; ++$day ) {
			...
			if ( !empty($posts[$day]) )
				$calendar_output .= '<a class="day_wrap" href="' . esc_url(!empty($posts[$day]['link']) ? $posts[$day]['link'] : get_day_link($thisyear, $thismonth, $day)) . '" title="' . esc_attr( is_int($posts[$day]['titles']) ? $posts[$day]['titles'].' '.esc_html__('items', 'globallogistics') : $posts[$day]['titles'] ) . "\">$day</a>";
			else
...
if ( !function_exists( 'themerex_callback_calendar_change_month' ) ) {
	function themerex_callback_calendar_change_month() {
		...
		$m = (int) themerex_get_value_gp('month');
		$y = (int) themerex_get_value_gp('year');
		$pt = themerex_get_value_gp('post_type');
...
if (!function_exists('themerex_output_single_comment')) {
	function themerex_output_single_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ) {
			case 'pingback' :
				?>
				<li class="trackback"><?php esc_html_e( 'Trackback:', 'globallogistics' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'globallogistics' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			case 'trackback' :
				?>
				<li class="pingback"><?php esc_html_e( 'Pingback:', 'globallogistics' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( 'Edit', 'globallogistics' ), '<span class="edit-link">', '<span>' ); ?>
				<?php
				break;
			default :
				...
				<li id="comment-<?php comment_ID(); ?>" <?php comment_class('comment_item'); ?>>
					...
					<div class="comment_content">
						<div class="comment_info">
							...
							<span class="comment_date"><span class="comment_date_label"><?php esc_html_e('on', 'globallogistics'); ?></span> <span class="comment_date_value"><?php echo get_comment_date(get_option('date_format')); ?></span></span>
						...
						<div class="comment_text_wrap">
							...
							<div class="comment_not_approved"><?php esc_html_e( 'Your comment is awaiting moderation.', 'globallogistics' ); ?></div>
							<?php } ?>
...
if (!function_exists('themerex_show_post_layout')) {
	function themerex_show_post_layout($opt = array(), $post_data=null, $post_obj=null) {
		...
		if (function_exists($func_name)) {
			...
			if(function_exists('themerex_sc_clear_dedicated_content')) themerex_sc_clear_dedicated_content();
		}
		return $layout;
	}
}
...
if (!function_exists('themerex_get_post_data')) {
	function themerex_get_post_data(&$opt, $post_obj=null) {
		...
		$opt['dedicated'] = function_exists('themerex_sc_get_dedicated_content') ? themerex_sc_get_dedicated_content() : '';
		...
		if (!empty($opt['dedicated'])) {
			...
			if (!is_singular() || themerex_get_global('blog_streampage') || (function_exists('themerex_sc_in_shortcode_blogger') && themerex_sc_in_shortcode_blogger(true)) || (themerex_strpos($class, 'sc_align')!==false && themerex_strpos($class, 'columns')===false)) {
...
// AJAX: Set post likes/views count
// Handler of add_action('wp_ajax_post_counter', 			'themerex_callback_post_counter');
// Handler of add_action('wp_ajax_nopriv_post_counter',	'themerex_callback_post_counter');
if ( !function_exists( 'themerex_callback_post_counter' ) ) {
	function themerex_callback_post_counter() {
		...
		$id = (int) themerex_get_value_gp('post_id');
		if (isset($_REQUEST['likes'])) {
			$counter = max(0, (int) themerex_get_value_gp('likes'));
			themerex_set_post_likes($id, $counter);
		} else if (isset($_REQUEST['views'])) {
			$counter = max(0, (int) themerex_get_value_gp('views'));
