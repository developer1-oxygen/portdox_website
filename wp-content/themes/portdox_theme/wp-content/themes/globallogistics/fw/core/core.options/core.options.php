...
if ( !function_exists( 'themerex_options_admin_menu_item' ) ) {
	...
	function themerex_options_admin_menu_item() {
		...
		add_theme_page(esc_html__('Theme Options', 'globallogistics'), esc_html__('Theme Options', 'globallogistics'), 'edit_theme_options', 'themerex_options', 'themerex_options_page');
	}
}
...
if ( !function_exists( 'themerex_get_custom_option' ) ) {
	function themerex_get_custom_option($name, $defa=null, $post_id=0, $post_type='post', $tax_id=0, $tax_type='category') {
		if (isset($_GET[$name]))
			$rez = themerex_get_value_gp($name);
		else {
			global $THEMEREX_GLOBALS;
...
if ( !function_exists( 'themerex_options_prepare_scripts' ) ) {
	...
	function themerex_options_prepare_scripts($override='') {
        ...
        themerex_storage_merge_array('js_vars', 'to_strings', array(
            'del_item_error' => esc_html__("You can't delete last item! To disable it - just clear value in field.", 'globallogistics'),
            'del_item' => esc_html__("Delete item error!", 'globallogistics'),
            'recompile_styles' => esc_html__("When saving color schemes and font settings, recompilation of .less files occurs. It may take from 5 to 15 secs dependning on your server's speed and size of .less files.", 'globallogistics'),
            'wait' => esc_html__("Please wait a few seconds!", 'globallogistics'),
            'reload_page' => esc_html__("After 3 seconds this page will be reloaded.", 'globallogistics'),
            'save_options' => esc_html__("Options saved!", 'globallogistics'),
            'reset_options' => esc_html__("Options reset!", 'globallogistics'),
            'reset_options_confirm' => esc_html__("Do you really want reset all options to default values?", 'globallogistics'),
            'reset_options_complete' => esc_html__("Settings are reset to their default values.", 'globallogistics'),
            'export_options_header' => esc_html__("Export options", 'globallogistics'),
            'export_options_error' => esc_html__("Name for options set is not selected! Export cancelled.", 'globallogistics'),
            'export_options_label' => esc_html__("Name for the options set:", 'globallogistics'),
            'export_options_label2' => esc_html__("or select one of exists set (for replace):", 'globallogistics'),
            'export_options_select' => esc_html__("Select set for replace ...", 'globallogistics'),
            'export_empty' => esc_html__("No exported sets for import!", 'globallogistics'),
            'export_options' => esc_html__("Options exported!", 'globallogistics'),
            'export_link' => esc_html__("If need, you can download the configuration file from the following link: %s", 'globallogistics'),
            'export_download' => esc_html__("Download theme options settings", 'globallogistics'),
            'import_options_label' => esc_html__("or put here previously exported data:", 'globallogistics'),
            'import_options_label2' => esc_html__("or select file with saved settings:", 'globallogistics'),
            'import_options_header' => esc_html__("Import options", 'globallogistics'),
            'import_options_error' => esc_html__("You need select the name for options set or paste import data! Import cancelled.", 'globallogistics'),
            'import_options_failed' => esc_html__("Error while import options! Import cancelled.", 'globallogistics'),
            'import_options_broken' => esc_html__("Attention! Some options are not imported:", 'globallogistics'),
            'import_options' => esc_html__("Options imported!", 'globallogistics'),
            'import_dummy_confirm' => esc_html__("Attention! During the import process, all existing data will be replaced with new.", 'globallogistics')
        ));
	}
}
		...
		<?php }	?>
				<div class="themerex_options_header">
					<div id="themerex_options_logo" class="themerex_options_logo">
						...
						<h2><?php esc_html_e('Theme Options', 'globallogistics'); ?></h2>
					</div>
					<div class="themerex_options_button_import"><span class="iconadmin-download"></span><?php esc_html_e('Import', 'globallogistics'); ?></div>
					<div class="themerex_options_button_export"><span class="iconadmin-upload"></span><?php esc_html_e('Export', 'globallogistics'); ?></div>
		<?php if ($to_flags['show_page_layout']) { ?>
					<div class="themerex_options_button_reset"><span class="iconadmin-spin3"></span><?php esc_html_e('Reset', 'globallogistics'); ?></div>
					<div class="themerex_options_button_save"><span class="iconadmin-check"></span><?php esc_html_e('Save', 'globallogistics'); ?></div>
		<?php }	?>
				</div>
	...
	function themerex_options_show_field($id, $field, $value=null) {
		...
		if ($field['type']=='media') {
			if (!isset($field['before'])) {
				$field['before'] = array(
					'title' => esc_html__('Choose image', 'globallogistics'),
					'action' => 'media_upload',
					'type' => 'image',
					'multiple' => false,
					'linked_field' => '',
					'captions' => array('choose' => esc_html__( 'Choose image', 'globallogistics'),
										'update' => esc_html__( 'Select image', 'globallogistics')
										)
				);
			}
		...
		if (themerex_options_is_group($field['type'])) {					// Close nested containers
			...
			if ($field['type'] != 'hidden') {
				...
				echo '<label class="themerex_options_field_label'.(!empty($THEMEREX_GLOBALS['to_flags']['add_inherit']) && isset($field['std']) ? ' themerex_options_field_label_inherit' : '').'" for="'.esc_attr($id).'">' . ($field['title']) 
					. (!empty($THEMEREX_GLOBALS['to_flags']['add_inherit']) && isset($field['std']) ? '<span id="'.esc_attr($id).'_inherit" class="themerex_options_button_inherit'.($inherit ? '' : ' themerex_options_inherit_off').'" title="' . esc_html__('Unlock this field', 'globallogistics') . '"></span>' : '')
					. '</label>'
					. "\n";
		...
		foreach ($field['val'] as $clone_num => $clone_val) {
			...
			case 'media':
				...
				if (!empty($clone_val) && !themerex_is_inherit_option($clone_val)) {
					...
					echo '<a class="themerex_options_image_preview" data-rel="popup" target="_blank" href="'.esc_url($clone_val).'">'.(!empty($ext) && themerex_strpos('jpg,png,gif', $ext)!==false ? '<img src="'.esc_url($clone_val).'" alt="'.esc_attr__('img', 'globallogistics').'" />' : '<span>'.($info['basename']).'</span>').'</a>';
				}
			break;
		...
		if (!themerex_options_is_group($field['type']) && $field['type'] != 'hidden') {
			if ($cloneable) {
				echo '<div class="themerex_options_input_button themerex_options_clone_button themerex_options_clone_button_add">'. esc_html__('+ Add item', 'globallogistics') .'</div>';
			}
			if (!empty($THEMEREX_GLOBALS['to_flags']['add_inherit']) && isset($field['std']))
				echo  '<div class="themerex_options_content_inherit"'.($inherit ? '' : ' style="display:none;"').'><div>'.esc_html__('Inherit', 'globallogistics').'</div><input type="hidden" name="'.esc_attr($id).'_inherit" value="'.esc_attr($inherit).'" /></div>';
			echo '</div>';
			if (!empty($field['desc']))
...
if ( !function_exists( 'themerex_options_save' ) ) {
	...
	function themerex_options_save() {
		...
		$mode = themerex_get_value_gp('mode');
		$override = $_POST['override']=='' ? 'general' : themerex_get_value_gp('override');
		...
		if ($mode=='export') {
			$name  = trim(chop(themerex_get_value_gp('name')));
			$name2 = isset($_POST['name2']) ? trim(chop(themerex_get_value_gp('name2'))) : '';
			$key = $name=='' ? $name2 : $name;
...
if ( !function_exists( 'themerex_options_import' ) ) {
	...
	function themerex_options_import() {
		...
		$override = $_POST['override']=='' ? 'general' : themerex_get_value_gp('override');
		$text = stripslashes(trim(chop(themerex_get_value_gp('text'))));
		if (!empty($text)) {
			...
			$key = trim(chop(themerex_get_value_gp('name2')));
			$import = get_option('themerex_options_'.($override), array());
			$opt = isset($import[$key]) ? $import[$key] : false;
		}
        $response = array('error'=>$opt===false ? esc_html__('Error while unpack import data!', 'globallogistics') : '', 'data'=>$opt);
		echo json_encode($response);
