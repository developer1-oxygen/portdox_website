	...
	function themerex_tribe_events_get_blog_title($title, $page) {
		...
		if ( themerex_strpos($page, 'tribe')!==false ) {
			if ( $page == 'tribe_category' ) {
				...
				$title = sprintf( esc_html__( 'Venue: %s', 'globallogistics' ), tribe_get_venue());
			} else if ( $page == 'tribe_organizer' ) {
				$title = sprintf( esc_html__( 'Organizer: %s', 'globallogistics' ), tribe_get_organizer());
			} else if ( $page == 'tribe_day' ) {
				$title = sprintf( esc_html__( 'Daily Events: %s', 'globallogistics' ), date_i18n(tribe_get_date_format(true), strtotime(get_query_var( 'start_date' ))) );
			} else if ( $page == 'tribe_month' ) {
				$title = sprintf( esc_html__( 'Monthly Events: %s', 'globallogistics' ), date_i18n(tribe_get_option('monthAndYearFormat', 'F Y' ), strtotime(tribe_get_month_view_date())));
			} else if ( $page == 'tribe_event' ) {
				$title = themerex_get_post_title();
			} else {
				$title = esc_html__( 'Tribe Events', 'globallogistics' );
			}
		}
		return $title;
	}
	...
	function themerex_tribe_events_get_stream_page_title($title, $page) {
		...
		if (themerex_strpos($page, 'tribe')!==false) {
			if (($page_id = themerex_tribe_events_get_stream_page_id(0, $page)) > 0)
				...
				$title = esc_html__( 'All Events', 'globallogistics');
		}
		return $title;
	}
}
...
if ( !function_exists( 'themerex_tribe_events_is_taxonomy' ) ) {
	...
	function themerex_tribe_events_is_taxonomy($tax, $query=null) {
		if (!empty($tax))
			...
			return $query && isset($query->tribe_is_event_category) && $query->tribe_is_event_category || is_tax(Tribe__Events__Main::TAXONOMY) ? 'courses_group' : '';
	}
}
...
if ( !function_exists( 'themerex_tribe_events_list_post_types' ) ) {
	...
	function themerex_tribe_events_list_post_types($list) {
		$list['tribe_events'] = esc_html__('Events', 'globallogistics');
		return $list;
	}
}
