...
if ( !function_exists( 'themerex_core_theme_setup' ) ) {
	...
	function themerex_core_theme_setup() {
		...
		load_theme_textdomain( 'globallogistics', themerex_get_folder_dir('languages') );
...
function themerex_core_init_theme() {
	...
	// Load skin
	$skin = sanitize_file_name(themerex_get_custom_option('theme_skin'));
...
if ( !function_exists( 'themerex_get_logo_icon' ) ) {
	function themerex_get_logo_icon($slug) {
		global $THEMEREX_GLOBALS;
		$skin = sanitize_file_name($THEMEREX_GLOBALS['theme_skin']);
		$logo_icon = themerex_get_custom_option($slug);
		if (empty($logo_icon) && file_exists(themerex_get_file_dir('skins/' . ($skin) . '/images/' . ($slug) . '.png')))
...
if ( !function_exists( 'themerex_register_theme_menus' ) ) {
	function themerex_register_theme_menus() {
		register_nav_menus(apply_filters('themerex_filter_add_theme_menus', array(
			'menu_main' => esc_html__('Main menu', 'globallogistics'),
			'menu_user' => esc_html__('User Menu', 'globallogistics'),
			'menu_side' => esc_html__('Side Menu', 'globallogistics')
		)));
	}
}
...
if ( !function_exists( 'themerex_enqueue_gallery' ) ) {
	function themerex_enqueue_gallery() {
		if (themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('css/__packed.css'))) {
			wp_enqueue_style( 'themerex-gallery-style',		themerex_get_file_url('js/gallery/component.css'), array(), null );
		}
		if (themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('js/__packed.js'))) {
			wp_enqueue_script( 'modernizr',	themerex_get_file_url('js/gallery/modernizr.min.js'),  array('jquery'), null, true );
			wp_enqueue_script( 'classie-script',	themerex_get_file_url('js/gallery/classie.js'),  array('jquery'), null, true );
			wp_enqueue_script( 'photostack-script',	themerex_get_file_url('js/gallery/photostack.js'),  array('jquery'), null, true );
		}
	}
}
...
if ( !function_exists( 'themerex_enqueue_diagram' ) ) {
	function themerex_enqueue_diagram($type='all') {
		if (themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('js/__packed.js'))) {
			if ($type=='all' || $type=='pie' || $type=='pie_2') wp_enqueue_script( 'diagram-chart-script',	themerex_get_file_url('js/diagram/chart.min.js'), array(), null, true );
...
if ( !function_exists( 'themerex_enqueue_formstyler' ) ) {
	function themerex_enqueue_formstyler() {
		if (themerex_get_theme_option('packed_scripts')=='no' || !file_exists(themerex_get_file_dir('js/__packed.js'))) {
			wp_enqueue_script( 'formstyler-script',	themerex_get_file_url('js/formstyler/jquery.formstyler.js'), array(), null, true  );
		}
	}
}
...
if ( !function_exists( 'themerex_core_frontend_scripts_inline' ) ) {
	function themerex_core_frontend_scripts_inline() {
        ...
        $code = themerex_get_global('js_code');
        if (!empty($code)) {
            $st = '<';
            $ct = '/';
            $et = '>';
...
if (!function_exists('themerex_core_add_scripts_inline')) {
	function themerex_core_add_scripts_inline($vars = array()) {
        ...
        if (is_singular() && themerex_get_theme_option('use_ajax_views_counter')=='yes') {
            ...
            $vars['views'] = (int) themerex_get_post_views(get_the_ID()) + 1;
        }
        ...
        // Theme base font
        $vars['theme_font'] = themerex_get_custom_option('typography_custom')=='yes' ? themerex_get_custom_option('typography_p_font') : '';

        // Theme skin
        $vars['theme_skin'] = esc_attr(themerex_get_custom_option('theme_skin'));
        $vars['theme_skin_bg'] = apply_filters('themerex_filter_get_theme_bgcolor', '');
        ...
        // Fix main menu
        $vars['menu_fixed'] = themerex_get_theme_option('menu_position')=='fixed';
        ...
        $vars['popup_gallery'] = themerex_get_theme_option('popup_gallery')=='yes';
...
if ( !function_exists( 'themerex_wp_title' ) ) {
	function themerex_wp_title( $title, $sep ) {
		...
		if ( $paged >= 2 || $page >= 2 )
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'globallogistics' ), max( $paged, $page ) );
		return $title;
	}
}
...
if ( !function_exists( 'themerex_callback_frontend_editor_save' ) ) {
	function themerex_callback_frontend_editor_save() {
		...
		$response['error'] = esc_html__('Post update denied!', 'globallogistics');
...
if ( !function_exists( 'themerex_callback_frontend_editor_delete' ) ) {
	function themerex_callback_frontend_editor_delete() {
		...
		$post_id = themerex_get_value_gp('post_id');

		$response['error'] = esc_html__('Post delete denied!', 'globallogistics');
    ...
    function themerex_importer_set_options($options=array()) {
        if (is_array($options)) {
            ...
            if ( is_dir(THEMEREX_THEME_PATH . 'demo/') ) {
                ...
                $options['demo_url'] = esc_url(themerex_get_protocol().'://demofiles.themerex.net/globallogistics/'); // Demo-site domain
            }
            ...
            // Default demo
            $options['files']['default']['title'] = esc_html__('Global Logistics Demo', 'globallogistics');
            $options['files']['default']['domain_dev'] = esc_url(themerex_get_protocol().'://globallogistics.dv.themerex.net');	// Developers domain
            $options['files']['default']['domain_demo']= esc_url(themerex_get_protocol().'://globallogistics.themerex.net');	// Demo-site domain
