...
if (!function_exists('themerex_get_date_or_difference')) {
	function themerex_get_date_or_difference($dt1, $dt2=null, $max_days=-1) {
		...
		if (abs($days) < $max_days)
			return sprintf($days >= 0 ? esc_html__('%s ago', 'globallogistics') : esc_html__('after %s', 'globallogistics'), themerex_get_date_difference($days >= 0 ? $dt1 : $dt2, $days >= 0 ? $dt2 : $dt1));
		else
...
if (!function_exists('themerex_get_date_difference')) {
	function themerex_get_date_difference($dt1, $dt2=null, $short=1, $sec = false) {
		...
		$rez = '';
		if ($months > 0 && $short == 2)
			$rez .= ($rez!='' ? ' ' : '') . sprintf($months > 1 ? esc_html__('%s months', 'globallogistics') : esc_html__('%s month', 'globallogistics'), $months);
		if ($days > 0 && ($short < 2 || $rez==''))
			$rez .= ($rez!='' ? ' ' : '') . sprintf($days > 1 ? esc_html__('%s days', 'globallogistics') : esc_html__('%s day', 'globallogistics'), $days);
		if ((!$short || $rez=='') && $hours > 0)
			$rez .= ($rez!='' ? ' ' : '') . sprintf($hours > 1 ? esc_html__('%s hours', 'globallogistics') : esc_html__('%s hour', 'globallogistics'), $hours);
		if ((!$short || $rez=='') && $min > 0)
			$rez .= ($rez!='' ? ' ' : '') . sprintf($min > 1 ? esc_html__('%s minutes', 'globallogistics') : esc_html__('%s minute', 'globallogistics'), $min);
		if ($sec || $rez=='')
			$rez .=  $rez!='' || $sec ? (' ' . sprintf($diff > 1 ? esc_html__('%s seconds', 'globallogistics') : esc_html__('%s second', 'globallogistics'), $diff)) : esc_html__('less then minute', 'globallogistics');
		return $rez;
	}
}
...
if (!function_exists('themerex_get_date_translations')) {
	function themerex_get_date_translations($dt) {
		return str_replace(
			...
			array(
				esc_html__('January', 'globallogistics'),
				esc_html__('February', 'globallogistics'),
				esc_html__('March', 'globallogistics'),
				esc_html__('April', 'globallogistics'),
				esc_html__('May', 'globallogistics'),
				esc_html__('June', 'globallogistics'),
				esc_html__('July', 'globallogistics'),
				esc_html__('August', 'globallogistics'),
				esc_html__('September', 'globallogistics'),
				esc_html__('October', 'globallogistics'),
				esc_html__('November', 'globallogistics'),
				esc_html__('December', 'globallogistics'),
				esc_html__('Jan', 'globallogistics'),
				esc_html__('Feb', 'globallogistics'),
				esc_html__('Mar', 'globallogistics'),
				esc_html__('Apr', 'globallogistics'),
				esc_html__('May', 'globallogistics'),
				esc_html__('Jun', 'globallogistics'),
				esc_html__('Jul', 'globallogistics'),
				esc_html__('Aug', 'globallogistics'),
				esc_html__('Sep', 'globallogistics'),
				esc_html__('Oct', 'globallogistics'),
				esc_html__('Nov', 'globallogistics'),
				esc_html__('Dec', 'globallogistics'),
			),
			$dt);
	}
}
