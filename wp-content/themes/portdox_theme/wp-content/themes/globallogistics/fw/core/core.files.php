		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (themerex_sc_param_is_on(themerex_get_theme_option('debug_mode')))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Put contents to the file "%s" failed', 'globallogistics'), $file));
			}
		}
		return false;
	}
	...
	function themerex_fgc($file) {
		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (themerex_sc_param_is_on(themerex_get_theme_option('debug_mode')))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get contents from the file "%s" failed', 'globallogistics'), $file));
			}
		}
		return '';
	}
	...
	function themerex_fga($file) {
		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (themerex_sc_param_is_on(themerex_get_theme_option('debug_mode')))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get rows from the file "%s" failed', 'globallogistics'), $file));
			}
		}
		return array();
	}
	...
	function themerex_get_list_folders($folder, $only_names=true) {
        ...
        if (isset($wp_filesystem) && is_object($wp_filesystem)) {
            ...
            if (themerex_get_theme_option('debug_mode')=='yes')
                throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get list folders from folder "%s" failed', 'globallogistics'), $dir));
        }
        return $list;
	}
}
...
if (!function_exists('themerex_autoload_folder')) {	
	function themerex_autoload_folder($folder, $from_subfolders=true, $from_skin=true) {
		...
		if ($from_skin && empty($skin_dir) && function_exists('themerex_get_custom_option')) {
			$skin_dir = sanitize_file_name(themerex_get_custom_option('theme_skin'));
...
if (!function_exists('themerex_get_socials_dir')) {	
	function themerex_get_socials_dir($soc, $return_url=false) {
		return themerex_get_file_dir('images/socials/' . sanitize_file_name($soc) . (themerex_strpos($soc, '.')===false ? '.png' : ''), $return_url, true);
	}
}
...
if (!function_exists('themerex_get_template_dir')) {	
	function themerex_get_template_dir($tpl) {
		return themerex_get_file_dir('templates/' . sanitize_file_name($tpl) . (themerex_strpos($tpl, '.php')===false ? '.php' : ''));
	}
}
?>